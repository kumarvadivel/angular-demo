...
export class FetchFlowApiService {
  ...
  fetchFlowApiCalls(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode) {
    ...
    switch(fetchflow.fetchflow){
      case 'multiFacilityRetreive':
        this.multifacilityRetrieve(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode)
        break;
      case 'fetchEligibilityDetails':
        this.fetchEligibilityDetails(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode);
        break;
      case 'fetchMultiFacilityList':
        this.theme1ApiService.facilityDetailsList(fetchflow.mappedParameters).then((res:any)=>{
          let journey =  this.getJourney()
          journey.metaData.capturedData.fetchMultiFacilityList = {...res}
          ...
          this.fetchflowErrorResponseDirectionMapping({fetchflow,totalLength,currentIndex,scope,fetchflowList,pageconfig,err,pageCode})
        })
        break;
      case 'retrieve':
        this.fetchRetrieve(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode)
        break;
      default:
  ...
  multifacilityRetrieve(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode){
    this.theme1ApiService.retreiveFacilityDetails(fetchflow.mappedParameters).then((res:any)=>{
      let journey =  this.getJourney()
      journey.metaData.capturedData.multiFacilityRetreive = {...res}
      this.setJourney(journey)
      this.fetchflowResponseDirection({
        fetchflow,
        totalLength,
  ...
  fetchEligibilityDetails(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode){
    this.theme1ApiService.fetchEligilibilityDetails(fetchflow.mappedParameters).then((res:any)=>{
      let journey =  this.getJourney()
      journey.metaData.capturedData.eligibilityDetails = {...res}
      this.setJourney(journey)
      this.fetchflowResponseDirection({
        fetchflow,
        totalLength,
  ...
  fetchRetrieve(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode){
    this.theme1ApiService.retrieve(fetchflow.mappedParameters).then((res:any)=>{
      let journey =  this.getJourney()
      for (let list of res.companyRepresentativeList) {
        if (list.keyPerson) {
          journey.metaData.capturedData.keyPersonDetails = list
          list.firstName = this.toCheckNullOrUndefined(list.firstName);
          list.directorTextVariable23 = this.toCheckNullOrUndefined(list.directorTextVariable23);
          list.lastName = this.toCheckNullOrUndefined(list.lastName);
          journey.metaData.capturedData.keyPersonFullName = list.firstName + " " + list.directorTextVariable23 + " " + list.lastName
        }
      }
      this.setJourney(journey)
      this.fetchflowResponseDirection({
  ...
  fetchflowErrorResponseDirectionMapping({ fetchflow,
    totalLength,
    currentIndex,
    scope,
    fetchflowList,
