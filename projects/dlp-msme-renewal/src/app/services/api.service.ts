...
import { Formatters } from '@msme-app/shared/helpers/Formatters';
...
export class ApiService {
  ...
  CHECK_SMA_FLAG_STATUS = "/finacle/smaFlagCheck";
  SAVE_MSME_SECURITY = `/api-v3/businessPatner/saveMSMESecurityDocumentAndUpdateContract`;
  ...
  breEligiblityCheck(params) {
    ...
    newParams["applicant"] = JSON.stringify({
      "uuid": params.Uuid,
    })
  ...
  checkSmaFlagStatus(params) {
    ...
    let encrypted = this.encryptFormBodyData_JSON(body)
    return this.commonApiService.doPostApiCall(this.BASE_URL_MICROSERVICE + this.CHECK_SMA_FLAG_STATUS, encrypted.body, undefined, undefined, this.environmentService.configData.appConfig.isEncryption ? {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${params.microservicetoken}`,
      'clientApiKey': encrypted.plainiv
    }:{
  ...
  saveMsmeSecurityDcoument(params) {
    let body = {}
    for (const prop in params) {
      if (prop != 'url') {
        if (prop != 'microservicetoken') {
    ...
    let encrypted = this.encryptFormBodyData_JSON(body)
    return this.commonApiService.doPostApiCall(this.BASE_URL_MICROSERVICE + this.SAVE_MSME_SECURITY, encrypted.body, undefined, undefined, this.environmentService.configData.appConfig.isEncryption ? {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${params.microservicetoken}`,
      'clientApiKey': encrypted.plainiv
    }:{
