...
export class DynamicFieldsComponent implements OnInit {
    ...
    getGoldProductList(field) {
        ...
        if ((journey.product.productCode === 'LA776') && (field.commonpropertyType == 'BORROWER_PROFILE_VARIABLE5')) {
            if (field.commonpropertyType == 'STAR_GOLD') {
                let params = {
                    productGroup: field.commonpropertyType,
                    constitution: journey.productUserType.toUpperCase()
                }
                this.theme1ApiService.getProductListForProductGroup(params).then((res: any) => {
                    if (res.code == 200) {
                        field.options = this.commonService.dataparsertoOptions(res.loanProductList);
                        this.dynamicGroupList = this.copySearchArray(this.dynamicGroupList);
                    }
                })
            }
    ...
    runValidationJson(field, initialLoad?, skip_calculation?, customDynamicList?,initialLoadRulesFor?) {
        for (const specifiers in field.validationJson) {
            switch (specifiers) {
                ...
                case 'property':
                    ...
                    break;
                case 'apiEndpoint':
                    field = this.runValidationApiEndpoint(field, initialLoad, customDynamicList, skip_calculation,initialLoadRulesFor);
                    break;
                case 'currentDate':
    ...
    runValidationApiEndpoint(field, initialLoad, customDynamicList, skip_calculation,initialLoadRulesFor) {
        if(initialLoadRulesFor==undefined){
        ...
        if (logicvariables.some(e => e == field.fieldName)) {
            switch (field.fieldDataType) {
                ...
                case 'DATE':
                    ...
                    break;
                case 'TABLE':
                    if (field && field.tableFooterData && field.tableFooterData.value != null && field.tableFooterData.value != '') {
    ...
    formulamapping(item, _emit?, initialLoad?, skip_calculation?, depenedentCall?, customDynamicList?,initialLoadRulesFor?) {
        ...
        if (item.validationJson != null) {
            if (item.dontTriggerSelfValidation !== true) {
                this.runValidationJson(item, initialLoad, skip_calculation, customDynamicList,initialLoadRulesFor)
            } else {
        ...
        if (item.rulesFor != null && item.rulesFor.length != 0) {
            item.rulesFor.forEach(rule => {
                ...
                if (f != undefined) {
                    this.formulamapping(f, true, initialLoad, skip_calculation, true, customDynamicList,initialLoadRulesFor)
                }
            })
        }
    }
    ...
    mapFormForEditDropDowns(e) {
        ...
        if (this.options.mapOptionsBasedOnOptionName) {
            ...
            option = e.options.find(f => f.optionName == this.options.mappingFields[e.fieldName])
            e.value = this.options.mappingFields[e.fieldName]
            e.displayValue = option != null ? option.optionName : null
        }
        if (e.dependentField != null) {
    ...
    mapFormForEditTable(e) {
        ...
        e.value = valTab;
        e = this.mapformTable(e)
        e = this.calculateTableFooterdata(e, {})
        if(this.options.newIndexNoCC >= 0){
            e.rowActionData[this.options.newIndexNoCC].actions[2].isDisplay = true
        }
        if(this.options.newIndexNoTL >= 0){
            e.rowActionData[this.options.newIndexNoTL].actions[2].isDisplay = true
        }
        e = this.checkExistingTl(e);
        return e
    }
    mapformTable(e){
        e.rowBinded = true
        e.rowActionData.forEach(da => {
            da.actions.forEach(dt => {
                if(this.options.reqEnhancment) {
                    switch(dt.actionCode){
                        case "EDIT":
                            dt.isDisplay = true;
                            break;
                        case "ACCEPT":
        ...
        e = this.checkFieldAccessModifier(e)
        return e;
    }

    checkFieldAccessModifier(e){
    ...
    checkExistingTl(e) {
        if(this.options.reqEnhancment){
            for (let j = 0; j < this.options.mappingFields.facilityDetailsListTable.length; j++) {
                if (this.options.mappingFields.facilityDetailsListTable[j].accountType == "Existing" && this.options.mappingFields.facilityDetailsListTable[j].facilityType == "TERM_LOAN") {
                    e.rowActionData[j].actions[1].isDisplay = false
                }
            }
        }
        return e
    ...
    async mapStarGoldCommonProperty(e) {
        ...
        await this.theme1ApiService.getProductListForProductGroup(params).then((res: any) => {
            if (res.code == 200) {
                e.options = this.commonService.dataparsertoOptions(res.loanProductList);
                this.dynamicGroupList = this.copySearchArray(this.dynamicGroupList)
            }
        })
    }
    ...
    async mapgoldPursposeofAdvance(e) {
        ...
        await this.theme1ApiService.getPurposeOfLoan(params).then((res: any) => {
            if (res.code == 200) {
                e.options = this.commonService.parsePurposeofAdvancetoOptions(res.dropDownElements);
                this.dynamicGroupList = this.copySearchArray(this.dynamicGroupList)
            }
        });
    }
    ...
    openFaciltyPopup(field) {
        if(this.options.externalFeedDataforValidationJson){
            field.popupConfig.forEach(f=>{
                if(f.componentType=='FORM'){
                    f.sectionContent.options.externalFeedDataforValidationJson = this.options.externalFeedDataforValidationJson
                }
            })
        }
        let dialogRef = this.dialog.open(PopupComponent, {
            panelClass: ['w-95', 'mob-w-100'],
            height: '95%',
            width: '90%',
            role:"dialog",
            ...
            data: {
                ...
                popupContent:  JSON.parse(JSON.stringify(field.popupConfig)),
                showBtns: true,
                validate: true,
                actionItems: true,
                flowAction: "create",
                submitFlow: field.submitFlow,
                code: field.code,
                newCcPopupConfig: this.options.newCCSection,
            }
        })
    ...
    addNewRow(field, tablefields, action?) {
        if(action?.actionType == "POPUP_ACTION"){
            action.isDisplay = false
            this.openFaciltyPopup(action);
        } else {
            // type == "TL" ? field.addMoreForTl = false : type == "CC" ? field.addMoreForCc = false : ""
            if (field.value.length > 0 && field.validateRowBeforeAdd) {
                if (field.rowActionData[field.value.length - 1].isCompleted === true) {
                    ...
                    // tablefields[0].value = type == "TL" ? "Term Loan" : type == "CC" ? "Cash Credit" : tablefields[0].value
                ...
                // tablefields[0].value = type == "TL" ? "Term Loan" : type == "CC" ? "Cash Credit" : null
                field.value.push(tablefields)
    ...
    tableActionDelete(_action, _tableRow, field, index) {
        if (_tableRow[0].value === "Term Loan") {
            field.addMoreForTl = true;
        } else if (_tableRow[0].value === "Cash Credit") {
            field.addMoreForCc = true;
        }
    ...
    checkProposedLimit(field, tableRow, index){
        if(field.value[index][0].value == "Term Loan"){
            let enterLimit = field.value[index].find(row => row.fieldName == "requestedLimit")
            if(enterLimit.value <= this.options.externalFeedDataforValidationJson.fetchEligibilityDetails.output.assessmentEligibility.tlEligibilityRound){
                return true
            } else {
                this.sharedService.openSnackBar('Term Loan proposed amount should not be greater than Term Loan eligiblity amount ' + this.options.externalFeedDataforValidationJson.fetchEligibilityDetails.output.assessmentEligibility.tlEligibilityRound, 'error', 200);
                return false
            }
        } else if(field.value[index][0].value == "Cash Credit") {
            let enterLimit = field.value[index].find(row => row.fieldName == "requestedLimit")

            if(enterLimit.value <= this.options.externalFeedDataforValidationJson.fetchEligibilityDetails.output.assessmentEligibility.ccEligibilityRound){
                this.callMultiFacilityUpdate(tableRow);
                return true
            } else {
                this.openCCEligibilityRound(tableRow)
                return true
            }
        }
    ...
    tableActionAccept(action, tableRow, field, index) {
        if (field.fieldName == "facilityDetailsListTable") {
            if (this.checkTableFilled(field, index)) {
                if (this.checkProposedLimit(field, tableRow, index)) {
                    return this.processFacilityDetailsTable(action, tableRow, field, index)
                }
            }
    ...
    processFacilityDetailsTable(action, tableRow, field, index){
        if (this.checkTableFilled(field, index)) {
            ...
            if(tableRow[0].value == "Cash Credit"){
                let journey = this.commonService.getJourney()
                journey.metaData.capturedData.editedCCProposedLimit = true;
                journey.metaData.capturedData.editedCCProposedLimitValue = tableRow[4].value;
                this.commonService.setJourney(journey)
            }
            return [field]
        }
    ...
    callMultiFacilityUpdate(tableRow, option?){
        let acctDetails = this.options.externalFeedDataforValidationJson.multiFacilityRetreive.responseAttr.facilityDetailsList.find( ob => ob.facilityDetailsTextVariable12 == tableRow[1].value)

        let params = {
            access_token : this.journey.product.access_token,
            uuid:acctDetails.uuid,
            loanFacilityUuid:acctDetails.loanFacilityUuid,
            loanPurposeUuid:acctDetails.loanPurposeUuid,
            loanUuid:acctDetails.loanUuid,
            facilityType: acctDetails.facilityType,
            requestedLimit: option == "No" ? this.options.externalFeedDataforValidationJson.fetchEligibilityDetails.output.assessmentEligibility.ccExistingLimit : tableRow[4].value,
            applicationSource:"WEB_JOURNEY"
        }
    ...
    openCCEligibilityRound(tableRow){
        let dialogRef = this.dialog.open(PopupComponent, {
            panelClass: ['w-30', 'mob-w-100'],
            height: '20%',
            disableClose: true,
            role:"dialog",
            ariaLabel:"CC Eligibility Round Section",
            data: {
              popupContent: this.options.CCEligibilityRoundSection,
              hideTitle: true,
              showBtns: false,
          ...
          dialogRef.afterClosed().subscribe(result => {
            if (result.action.actionLabel == 'Yes') {
                tableRow[4].selectYes = true;
            } 
            if (result.action.actionLabel == 'No') {
                tableRow[4].value = this.options.externalFeedDataforValidationJson.fetchEligibilityDetails.output.assessmentEligibility.ccExistingLimit
            }
            if(tableRow[0].value == "Cash Credit"){
                let journey = this.commonService.getJourney()
                journey.metaData.capturedData.editedCCProposedLimit = true;
            ...
            this.callMultiFacilityUpdate(tableRow, result.action.actionLabel);
          })
    }

    openFaciltyPopupForUpdate(field, row, index){
        let popupContent;
        let submitFlow;
        let code;
        switch(row[0].value){
            case 'Cash Credit':
                code = "CC";
                popupContent = field.tableFooterActions[1].popupConfig;
                submitFlow = field.tableFooterActions[1].submitFlow;
                popupContent[0].sectionContent.options.mapAndDisable = false
                popupContent[0].sectionContent.options.externalFeedDataforValidationJson = this.options.externalFeedDataforValidationJson
                popupContent[0].sectionContent.options.mappingFields = this.options.mappingFields.facilityDetailsListTable.find(lt => lt.accountType == "New" && lt.facilityType == "CASH_CREDIT")
                break;
            case 'Term Loan':
                code = "TL";
                popupContent = field.tableFooterActions[0].popupConfig;
                submitFlow = field.tableFooterActions[0].submitFlow;
                popupContent[0].sectionContent.options.mapAndDisable = false
                popupContent[0].sectionContent.options.externalFeedDataforValidationJson = this.options.externalFeedDataforValidationJson
                popupContent[0].sectionContent.options.mappingFields = this.options.mappingFields.facilityDetailsListTable.find(lt => lt.accountType == "New" && lt.facilityType == "TERM_LOAN")
                break;
        }
        let dialogRef = this.dialog.open(PopupComponent, {
            panelClass: ['w-95', 'mob-w-100'],
            ...
            data: {
                title: 'Facility Details',
                popupContent:  JSON.parse(JSON.stringify(popupContent)),
                showBtns: true,
                validate: true,
                actionItems: true,
                flowAction: "update",
    ...
    tableActionEdit(action, tableRow, field, index) {
        if((this.options?.newIndexNoCC || this.options?.newIndexNoTL) && (this.options.newIndexNoCC == index || this.options.newIndexNoTL == index)){
            this.processtableActionEdit(action, tableRow, field, index)
            this.openFaciltyPopupForUpdate(field, tableRow, index);
            return [field, action]
        } else {
            this.processtableActionEdit(action, tableRow, field, index)
            field.value[index].forEach(act => {
                if (act.fieldDataType == "ACTION") {
                    act.actions.forEach((curr) =>  curr.isDisplay = curr.actionCode==='ACCEPT')
                }
    ...
    processtableActionEdit(action, tableRow, field, index){
