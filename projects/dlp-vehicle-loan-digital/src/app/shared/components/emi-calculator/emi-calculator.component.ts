...
import {ConfigurationService} from '@vldigi-app/services/configuration.service';
...
export class EmiCalculatorComponent implements OnInit {
    ...
    validate(event,data){
        if(Number(event.target.value)>data.max) {
          event.target.value = data.max
          this.emiCalcData.loanAmount = this.emiCalcData.maxLoanAmount
          this.emiCalcData.tenure=this.emiCalcData.maxTenure
         this.valueEmit.emit(this.emiCalcData)
       }
      }
}
