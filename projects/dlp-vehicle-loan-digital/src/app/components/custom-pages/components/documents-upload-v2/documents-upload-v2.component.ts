...
import { ViewDocumentPopupComponent } from '@vldigi-app/shared/components/view-document-popup/view-document-popup.component';
...
export class DocumentUploadV2Component implements OnInit {
    ...
    readOrigin() {
        ...
        return getOrigin.includes('localhost') ? window.origin + "/#" : window.origin + '/star-digi-vehicle-loan' + "/#";
    }
    ...
    deleteDocument(doc,document,index){
        let params = {
          access_token:this.journey.product.access_token,
          supportingDocumentUuid:doc.documentUuid
        }
    
        this.theme1ApiService.deleteDoc(params).then((res:any)=>{
          if(res.responseAttr.success){
            this.sharedService.openSnackBar('Document Deleted Successfully')
            document.uploadDocuments.splice(index,1)
            if(document.uploadDocuments.length==0){
