...
import { FetchFlowApiService } from "@vldigi-app/services/fetch-flow-api.service";
import { InitializeJourneyService } from "@vldigi-app/services/initialize-journey.service";
import { MetaConfigService } from "@vldigi-app/services/meta-config.service";
import { LocalStorage } from "@vldigi-app/shared/helpers/localStorage";
import { SharedService } from "@vldigi-app/shared/services/utils/shared.service";
    ...
    if (snap && snap != null) {
      ...
      this.apiService.fetchBorrowerDetails(params).then((res: any) => {
        ...
        if (res.code == '200') {
          ...
          this.initializeJourneyService.initializeJourney(this.commonVariableService.loanProductInfo.find(o => o.productCode === 'SDVLN'), undefined, true, { "functionskel": this.vkycFlow, "arguments": this });
        }
