...
import { isEqual } from "@vldigi-app/shared/utils/utils";
...
export class InitializeJourneyService {
    ...
    initializeJourney(product, resumeJourney?, dontRouteTofirstPage?,initializercallback?) {
        ...
        if (this.commonVariableService.tenentConfiguration.FETCH_PRODUCT_CONFIG_FROM_JSON_FILE) {
            this.fetchAndSetProductConfigFromJson(journey, product, resumeJourney, dontRouteTofirstPage,initializercallback);
        } else {
            this.setProductConfigFromService(journey, product, resumeJourney, dontRouteTofirstPage,initializercallback);
        }
    }
    ...
    mapProductData(journey, dontRoute, reesumeJourney?,initializercallback?) {
    ...
    updateStepCompletion(stepData: any, nextStepData: any): { updatedStepData: any } {
        if (stepData.subStep) {
            ...
            if (updatedSubSteps.length === 0 && nextStepData) {
                nextStepData.isActive = true;
            }
        }
    ...
    mapDataFromParameter(params, dataCloud) {
        ...
        params.forEach((f) => {
            if (typeof f == "object") {
                if (f[1] == "STATIC") {
                    mappedParams[this.applyJsonLogic(f[0],dataCloud)] = f[2];
                } else {
                    if (f[1] == "APPEND_VALUES") {
                        let final = "";
                        final = this.mapAppendValuesParameter(final, f[2], dataCloud)
                        mappedParams[this.applyJsonLogic(f[0],dataCloud)] = final;
                    } else {
                        if (f[1] == "RULE_OUTPUT") {
                            mappedParams[this.applyJsonLogic(f[0],dataCloud)] = this.applyJsonLogic(f[2], dataCloud);
                        } else {
                            mappedParams[this.applyJsonLogic(f[0],dataCloud)] = this.ArrayMethods.getPathVal(
                                dataCloud,
    ...
    private appiceInitializer(product) {
        ...
        if (this.journey.product.productCode == 'SDVLN') {
            this.appiceEvent("PLVisit", product);
        }
    }
}
