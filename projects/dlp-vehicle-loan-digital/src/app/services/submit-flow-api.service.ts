...
import { LocalStorage } from '@vldigi-app/shared/helpers/localStorage';
...
handleErrorSubmitflow({ submitflow, currentIndex, totalLength, configList, pageconfig, pageCode, journey, noLoader }: { submitflow; currentIndex; totalLength; configList; pageconfig; pageCode; journey; noLoader?; }){
     return (err) => {
            ...
            switch (result) {
                ...
                case 'PROCEED':
                    if (lastIndex) {
                        ...
                        this.formSubmitEventExecutor(submitflow)
                        this.apiCall({ submitflow: configList.formSubmitFlow[currentIndex + 1], currentIndex: currentIndex + 1, totalLength, pageCode, configList, pageconfig, extraCloudParams, noLoader })
                    }
                    break;
                case 'ERROR':
    ...
    apiresponseValidator({
        ...
        if (currentIndex != totalLength - 1) {
            if (submitflow.validateResponse) {
                ...
                this.formSubmitEventExecutor(submitflow)
                if (journeyPage) {
                    this.apiCallJourneyPage({ submitflow: configList.formSubmitFlow[currentIndex + 1], currentIndex: currentIndex + 1, totalLength, pageCode, configList, pageconfig, extraCloudParams, noLoader })
                } else {
                    this.apiCall({ submitflow: configList.formSubmitFlow[currentIndex + 1], currentIndex: currentIndex + 1, totalLength, pageCode, configList, pageconfig, extraCloudParams, noLoader })
                }
            }
    ...
    apiCallJourneyPage({ submitflow, currentIndex, totalLength, pageCode, configList, pageconfig, extraCloudParams, noLoader }: { submitflow; currentIndex; totalLength; pageCode; configList; pageconfig?; extraCloudParams?; noLoader?; }) {
    ...
    apiCall({ submitflow, currentIndex, totalLength, pageCode, configList, pageconfig, extraCloudParams, noLoader }: { submitflow; currentIndex; totalLength; pageCode; configList; pageconfig?; extraCloudParams?; noLoader?; }) {
