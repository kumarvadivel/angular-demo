...
export class ApiService {
 ...
 VEHICLE_CREATE=`api-v3/vehicle/create`;
 ...
 generateMobileOtp(params){
  ...
  updateEmploymentDetails(params){
   this.employmentProfileUpdate(params)
  }
  ...
  createVehicleDetails(params){
    ...
    let encrypted = this.encryptFormBodyData_JSON(body)
    return this.commonApiService.doPostApiCall(this.BASE_URL_MICROSERVICE+this.VEHICLE_CREATE, encrypted.body, undefined, undefined, this.environmentService.configData.appConfig.isEncryption ? {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${params.microservicetoken}`,
      'clientApiKey': encrypted.plainiv
    }:{
