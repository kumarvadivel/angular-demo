...
import { Encrypt } from '@vldigi-app/shared/helpers/Encrypt';
...
import { CustomHttpParamEncoder } from '@vldigi-app/shared/helpers/customHttpEncoder';
import { Formatters } from '@vldigi-app/shared/helpers/Formatters';
@Injectable({
    providedIn: 'root'
})
export class CommonApiService {
    ...
    private processBody(res,url,resolve){
        let body = res.body
    ...
    public doGetApiCall(url, queryParams, customHeaders?) {
        return new Promise((resolve, reject) => {
            this.httpClient.get(url, {params: queryParams, headers: customHeaders || {},observe:"response"},).subscribe(
                (res:any) => {
                    this.processBody(res,url,resolve)
                },
                err => {
                    reject(err);
                }
