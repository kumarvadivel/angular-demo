...
export class DynamicFieldsComponent implements OnInit {
    ...
    ngOnChanges(simpleChange) {
        if(simpleChange.dynamicGroupList){
            if(!simpleChange.dynamicGroupList.firstChange){
                this.dynamicGroupList = this.groupFieldBasedongroup(this.dynamicGroupList)
                this.checkColumnConfig()
    ...
    runValidationApiEndpoint(field, initialLoad, customDynamicList, skip_calculation,initialLoadRulesForcall) {
    ...
    processAddressField(field, data) {
        field.addressFields = field.addressFields.map(addressField => {
            [addressField, data] = this.processAddressSubField(addressField, data,field)
            return addressField
        });
        return data
    }
    ...
    processAddressSubField(addressField, data,field) {
        ...
        if (addressField.fieldDataType === 'DROPDOWN' || addressField.fieldDataType === 'AUTO_COMPLETE') {
            ...
            data[field.fieldName][addressField.fieldName] = option?.optionKey || null;
        } else if (addressField.fieldDataType === 'DATE') {
            data[field.fieldName][addressField.fieldName] = addressField.value ? this.formatDate(addressField.value) : null;
        } else {
            data[field.fieldName][addressField.fieldName] = addressField.value;
        }

        if (this.addressFieldHasPrefix(addressField)) {
            data[field.fieldName][addressField.prefixfieldName] = addressField.prefix;
        }
    ...
    autoCompletePlaceholder(placeholder):any{
        let config = {
    ...
    removeKeyword(keyword: string,field) {
        const index = field.value.indexOf(keyword);
        if (index >= 0) {
            field.value.splice(index, 1);
        }
      }
