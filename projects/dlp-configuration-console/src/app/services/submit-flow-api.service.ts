...
handleErrorSubmitflow(submitflow,currentIndex,totalLength,configList,pageconfig,pageCode,journey,noLoader){
     return (err) => {
                ...
                case 'bureauCall':
                    setTimeout(() => {
                        this.theme1ApiService.cibilCheck(submitflow.mappedParameters).then((response) => {
                            ...
                            this.handleErrorSubmitflow(submitflow,currentIndex,totalLength,configList,pageconfig,pageCode,journey,noLoader)
                        );
                    }, 10000)
                    break;
                default:
            ...
            switch (result) {
                ...
                case 'PROCEED':
                    if (lastIndex) {
                        ...
                        this.formSubmitEventExecutor(submitflow)
                        this.apiCall(configList.formSubmitFlow[currentIndex + 1], currentIndex + 1, totalLength, pageCode, configList, pageconfig, extraCloudParams,noLoader)
                    }
                    break;
                case 'ERROR':
    ...
    apiCallJourneyPage(submitflow, currentIndex, totalLength, pageCode, configList, pageconfig?, extraCloudParams?,noLoader?) {
    ...
    apiCall(submitflow, currentIndex, totalLength, pageCode, configList, pageconfig?, extraCloudParams?,noLoader?) {
