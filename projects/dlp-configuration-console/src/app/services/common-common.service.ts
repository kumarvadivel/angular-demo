...
import { assign, uniqBy } from "@config-app/shared/utils/utils";
...
export class CommonCommonService {
    ...
    resetFormSectionValues(form){
        form.sectionContent.fields.forEach(f=>{
            if(f.fieldDataType!='ADDRESS'){
                f.value = null
            }else{
                f.value = null
                f.addressFields.forEach(af=>{
                    af.value = null
                })
            }
        })
