...
export class ConsentComponent implements OnInit {
  ...
  idConsentRemaps = {
    ...
    'CJ-CC-PrivacyPolicy':{
      'type':'POPUP',
      'consentCode':'PRIVACY_POLICY_AGRI_KCC_RENEWAL'
    },
    'PRIVACY_POLICY_VL':{
      'type':'POPUP',
      ...
      if(ev.target.id=='readButton'){
        ...
        if(ele.id=='readMore'){
          ...
          viewMore.classList.add('link_consent_dynamic')
           // Check if the element with ID 'viewLess' already exists
           let existingViewLess = document.getElementById('viewLess')
           if (!existingViewLess) {
  ...
  checkValidity(isFromDefault){
    if(this.field.options.every(t=>t.completed)){
      ...
      this.field.options.forEach(val=>{
        this.isInvalid =this.commonVariableService.isConsentValid =  !(val.completed==null && !isFromDefault);
      })
    }
  }
