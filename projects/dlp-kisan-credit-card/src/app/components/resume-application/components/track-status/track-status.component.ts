...
import { SharedService } from '@kcc-app/shared/services/utils/shared.service';
...
export class TrackStatusComponent implements OnInit {
  ...
  constructor(public commonService:CommonCommonService,private activeRoute: ActivatedRoute,private router:Router,
    public sharedService:SharedService,public resumeVariableService:ResumeTheme1VariableService,public localStorage: LocalStorage) { 
  ...
  otpCompleteTrigger($event){
    ...
    if(this.journey.product.loanDetailsWithAccessToken?.length == 0) {
      ...
      this.router.navigateByUrl('1/resume-journey/application-status?category=resume-application');
    } else if(this.journey.product.loanDetailsWithAccessToken?.length != 0 && this.categoryType == 'track-status') {
      this.router.navigateByUrl('1/resume-journey/application-status?category=track-status');
    }
  }
  }
