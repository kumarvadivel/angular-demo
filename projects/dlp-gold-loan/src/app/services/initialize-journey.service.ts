...
import { isEqual } from "@gl-app/shared/utils/utils";
...
export class InitializeJourneyService {
  ...
  private get isProductFile(): boolean {
  ...
  private mapProductData(journey, dontRoute, reesumeJourney?) {
    this.apiService
      .fetchLoanProducts()
      .then((res: any) => {
        let product;
  ...
  findNextpageCode(pageCode) {
    ...
    if (this.isProductFile) {
      ...
      try {
        pageSequence =
          productConfig.pageSequenceData["journeyPages"][
  ...
  createConsents(submitConsentData) {
    ...
    submitConsentData.forEach((data) => {
      ...
      this.apiService.createConsent(params).catch(console.error);
    });
  }
  ...
  navigateJourney(
    ...
    const sourceConfig = this.isProductFile
  ...
  private fetchAndSetProductConfigFromJson(
    ...
    this.apiService
      ...
      .then(async (response) => {
        ...
        let configData = await this.checkAndDecryptConfigData(response);
  ...
  private appiceInitializer(product) {
    ...
    this.appiceEvent("AGLVisit", product);
  }
