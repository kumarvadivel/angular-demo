...
export class SectionBuilderComponent {
      ...
      switch (section.componentType) {
        case "TITLE":
          section.sectionContent.titleData =
            section.sectionContent.titleData(feedData);
          break;
        case "PARAGRAPH":
          section.sectionContent.paragraphData =
            section.sectionContent.paragraphData(feedData);
          break;
      }
    });
    return config;
    ...
    switch (data.linkType) {
      ...
      case "REDIRECT":
        this.router
          ...
          .then(() => {
            this.router.navigateByUrl(`${data.url}`).catch(console.error);
          }).catch(console.error);
  ...
  otherEventsBind($event, section) {
    this.status.emit({
      ok: true,
      TriggerSection: section.componentType,
      data: $event,
    });
  }
}
