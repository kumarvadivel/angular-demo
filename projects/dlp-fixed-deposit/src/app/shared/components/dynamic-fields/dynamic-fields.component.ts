...
import { JourneyEventsService } from '@fd-app/services/journey-events.service';
import { InitializeJourneyService } from '@fd-app/services/initialize-journey.service';
import { ArrayMethod } from '@fd-app/shared/helpers/ArrayMethods';
import { DatePickerAdapter } from '@fd-app/shared/helpers/datePickerAdapter';
...
export class DynamicFieldsComponent implements OnInit {
  ...
  //oninput
  validationCheck(field, verificationTrigger?,_event?) {
...
mapHardCodingValuesForNonAddressFields(field,fieldNames_dontTrigger,fieldNames_alwaysRun){
  ...
  private mapchildOption_v1CommonMapper(e, data, option, valueresetted) {
    if (e.dependentField == data.fieldName) {
      ...
      if (e.value != null && e.value != undefined) {
        this.mapchildOption_v1CommonMappersubMethod(e)
      }
      if (
  ...
  mapchildOption_v1CommonMappersubMethod(e){
...
mapEndpointDropdownForEdit(source, field, initialLoad, skip_calculation):any {
  ...
  mapDataForDD(source,field,initialLoad,skip_calculation){
    if(typeof source[field["fieldName"]]=='object'){
      this.handleSourceFieldObjectchanges(source,field,initialLoad,skip_calculation);
       }else{
  ...
  handleSourceFieldObjectchanges(source,field,initialLoad,skip_calculation){
...
mapErrorsForEndpointLogic_address(errors,field,initialLoad,skip_calculation){
        ...
        row.forEach((field) => {
          if (
            ...
            field.fieldName === item.dependentField) {
              state = this.checkParentfilledForAutoComplete(field, state)
  ...
  checkParentfilledForAutoComplete(field, state){
    if(field.fieldDataType == 'AUTO_COMPLETE'){
  ...
  async mapDocumentType(e) {
    ...
    await this.apiService.getSupportingDoc(params).then((res: any) => {
      ...
      localStorage.setItem('REQUIRED_DOCUMENTS',JSON.stringify(res.requiredDocuments))
      this.commonVariableService.reqSupportingDocList = JSON.parse(localStorage.getItem('REQUIRED_DOCUMENTS'))
      this.commonService.getDocumentList(e)
      localStorage.setItem('REQUIRED_DOCUMENTS',JSON.stringify(res.requiredDocuments))
      this.commonVariableService.reqSupportingDocList = JSON.parse(localStorage.getItem('REQUIRED_DOCUMENTS'))
...
handleResponseVerificationFields(field,res){
  ...
  fileChange(field, event) {
    this.commonVariableService.uploaddone = false;
    this.commonVariableService.reqSupportingDocList.forEach(list => {
      if ("IND_PAP" === list.loanPurposeDocumentCategoryCode) {
        this.selectedDocumentData = list;
      }
    });
    this.selectedAddressType = this.findFieldNamevalue(field.fileDependentField, "ALL").value
    let aliasId;
    if (this.selectedDocumentData.alias) {
      const selectedType = this.selectedAddressType.toLowerCase();
      const documentTypeMappings = {
        ...
        "property tax receipt": "Rental Agreement or Lease Agreement",
        "copies of registered leave & license agreement/sale deed/lease agreement":"Copies of Registered Leave & License agreement/Sale Deed/Lease Agreement"
      };
    
      this.selectedDocumentData.alias.forEach(element => {
        if (documentTypeMappings[selectedType] === element.name) {
          aliasId = element.uuid;
        }
      });
    }
    if(this.commonVariableService.uploadedDoc){
      this.removeDocument(field).then((res: any) => {
        if (res) {
          this.uploadFieldDocument(field,this.selectedDocumentData,aliasId,event);
        }
      })  
    }else{
      this.uploadFieldDocument(field,this.selectedDocumentData,aliasId,event);
  ...
  uploadFieldDocument(field,selectedDocumentData,aliasId,event) {
    this.commonVariableService.globalLoaderState = true;
    let params = {
      access_token: this.journey.product.access_token,
      loanUuid: this.journey.product.loanUuid,
    ...
    this.apiService.uploadDocumentFile(params).then((res: any) => {
      if (res.code == "200") {
        this.handleSuccessfulUpload(field);
      } else {
        this.handleUploadFailure(field);
      }
    })
      .catch(() => {
        this.handleUploadFailure(field);
  ...
  handleSuccessfulUpload(field) {
    this.commonService.getDocumentList(field).then((data: any) => {
      if (data) {
        data.supportingDocuments.forEach((innerData: any) => {
          this.commonVariableService.reqSupportingDocList.forEach((item: any) => {
            if (innerData.DocumentCategoryUuid === item.documentTypeUuid &&
              item.loanPurposeDocumentCategoryCode === 'IND_PAP' &&
              innerData.documentUploadedSuccessfully &&
              innerData.size > 0) {
              field.value = innerData;
              this.validationCheck(field);
            }
          });
        });
  ...
  handleUploadFailure(field) {
    this.commonVariableService.uploaddone = null;
  ...
  removeDocument(field?){
    ...
    let params = {
      access_token: this.journey.product.access_token,
      supportingDocumentUuid: this.commonVariableService.uploadedDoc.documentUuid,
      microservicetoken: this.journey.product.oauthAccessToken
    }
    return new Promise((resolve, _reject) => {
      this.apiService.removeDocument(params)
        .then(
          resp => {
            resolve(resp);
            this.showloader = false
            this.commonVariableService.uploadedDoc=""
            field.value = null
