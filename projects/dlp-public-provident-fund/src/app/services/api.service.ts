...
import appConfiguration from '@ppf-app/app.config.json'
 ...
 BRANCH_STATE_MASTER =`/api-v3/restPublic/master-branch-list-new`;
  ...
  genericPostApiFetch(params) {
    ...
    if (params['microservicetoken']) {
      ...
      let encrypted = this.encryptFormBodyData_JSON(body);
      const headers = this.prepareHeaders(params.microservicetoken,encrypted);
  ...
  callPlatformEndpointFromObject(endPointData){
    ...
    return this.commonApiService.doPostApiCall(this.BASE_URL + endPointData.url,encrypted.body, this.environmentService.configData.appConfig.isEncryption ? {...this.commonVariableService.encryptionHeaders,clientApiKey:encrypted.plainiv} : undefined
      )
  }
