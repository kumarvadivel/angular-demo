...
export class SanctionComponent implements OnInit {
  ...
  contentModeratorWithRespectToProduct() {
    ...
    if(this.journey.product.productCode == "SML"){
      this.config[4].sectionContent.items[1].sectionContent.items[1].sectionContent.config.data[0].value = this.$scope.fetchRepaymentSchedule.object.principal
      this.config[4].sectionContent.items[1].sectionContent.items[1].sectionContent.config.data[1].value = this.$scope.fetchRepaymentSchedule.object.payableInterest
  ...
  sectionBuilderEmitter($event) {
    ...
    if (this.journey.product.productCode == "SML") {
      this.shishuSectionBuilderEmitter($event)
    }
  }
