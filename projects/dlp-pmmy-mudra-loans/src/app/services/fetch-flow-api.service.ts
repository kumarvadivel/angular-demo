...
import { EnvironmentService } from '@pmmy-environments/environment.service';
...
export class FetchFlowApiService {
  ...
  public mergeAllFormsInConfig(config) {
    let val = {}
    config.forEach(c => {
      if (c.componentType == 'FORM') {
        c.sectionContent.formValueEmitter.next()
        val = { ...val, ...c.sectionContent.formValue }
      }
      if(c.componentType=='GRID'){
        val = {...val,...this.mergeAllFormsInConfig(c.sectionContent.items)}
      }
    })
    return val
  }
