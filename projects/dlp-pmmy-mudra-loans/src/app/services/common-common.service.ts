...
import { ExportService } from "../shared/services/utils/export.service";
...
import { assign, cloneDeep, uniqBy } from "@pmmy-app/shared/utils/utils";
...
export class CommonCommonService {
  ...
  pageSectionContentModeration(config, dataCloud) {
      ...
      config.map((section) => {
          ...
          if(section.componentType=='GRID'){
            section.sectionContent.items = this.pageSectionContentModeration( section.sectionContent.items,dataCloud)
          }
          if (section.componentType == "FORM") {
              moderateFormData(section, dataCloud)
          }
          this.pageSectionContentModerationForTable(section, dataCloud)
      });
      return config;
  }

  pageSectionContentModerationForTable(section, dataCloud){
  ...
  findPreviouspageCode(pageCode, limitStepIdx?) {
      let journey = this.getJourney()
      let productConfig = this.initializeJourneyService.getLocalStoreProductConfiguration();
      let pageSequence = productConfig.pageSequenceData['journeyPages'][journey.productUserType]
      let prevpageCode = pageSequence[pageSequence.findIndex(r => r.pageCode == pageCode)]
  ...
  parseJourneyType(form){
      ...
      switch(form.constitution){
        ...
        case 'Sole Proprietorship':
            if(journey.product.productCode=="SML") {
                this.updateJourneyType('soleproprietorship')
                this.updateConstitution(form.constitution)
            } else {
    ...
    formDataEditDataPopulatorSimplify(section, metaConfig, data) {
        ...
        for (const prop in metaConfig.prepopulationRemaps) {
            if (Array.isArray(metaConfig.prepopulationRemaps[prop])) {
                this.prepopulationRemapsRuleOutput(metaConfig, prop, fieldStateConfig, data, mapping_Data);
    ...
    prepopulationRemapsRuleOutput(metaConfig, prop, fieldStateConfig, data, mapping_Data) {
  ...
  fetchSubFlowPageConfig(pageCode,journey,subFlow){
    let productConfig
    let temp
    if(this.commonVariableService.tenentConfiguration.FETCH_PRODUCT_CONFIG_FROM_JSON_FILE){
      ...
      temp = cloneDeep(productConfig.pageSectionConfig[journey.productUserType][subFlow.initiatedPage]['subFlowPageConfig'][subFlow.userType][pageCode])
      
    }else{
      temp = cloneDeep(this.commonVariableService.pageSectionConfig[journey.product.productCode][journey.productUserType][subFlow.initiatedPage]['subFlowPageConfig'][subFlow.userType][pageCode])
    }
    return temp
  }

  fetchSubFlowPagemetaConfig(pageCode,journey,subFlow){
    if(this.commonVariableService.tenentConfiguration.FETCH_PRODUCT_CONFIG_FROM_JSON_FILE){
      let productConfig= this.localStorage.SessionGetItem('PRODUCT_CONFIGURATION')
      let temp = cloneDeep(productConfig.pageMetaConfig[journey.productUserType][subFlow.initiatedPage]['subFlowPageMetaConfig'][subFlow.userType][pageCode])
      temp = this.parseMetaConfig(temp)
      return temp
    }else{
      return this.commonVariableService.pageMetaConfig[journey.product.productCode][journey.productUserType][subFlow.initiatedPage]['subFlowPageMetaConfig'][subFlow.userType][pageCode]
  ...
  proceedSubFlow(pageCode){
    let subFlow:any = this.localStorage.SessionGetItem('SUBFLOW')
    let index=subFlow.pageSequence.findIndex(p=>p.pageCode==pageCode) 
    if(index==subFlow.pageSequence.length-1){
      this.localStorage.SessionremoveItem('SUBFLOW')
      this.localStorage.SessionSetItem('COMPLETED_SUBFLOW_DATA',subFlow)
      this.NavigateJourney(subFlow.initiatedPage)
    }else{
      this.router.navigateByUrl(subFlow.pageSequence[index+1].url)
    }
  }

  executeAppiceSetUser() {
    let userObj = {};
    let curJourney = this.getJourney();
    let aadharData = curJourney.metaData.capturedData.aadharData;
    let udyamData = curJourney.metaData.capturedData.udyamData;
    if (aadharData) {
      userObj['name'] = aadharData.name;
      userObj['gender'] = aadharData.gender;
      userObj['dob'] = aadharData.DoB;
      userObj['phone'] = (curJourney?.metaData?.capturedData?.mobileNumber) ? (curJourney?.metaData?.capturedData?.mobileNumber) : undefined;
      userObj['email'] =  (aadharData?.alternativeUsername) ? (aadharData?.alternativeUsername) : undefined;
    }
    else if(udyamData) {
      userObj['name'] = udyamData.companyName;
      userObj['phone'] = (curJourney?.metaData?.capturedData?.mobileNumber) ? (curJourney?.metaData?.capturedData?.mobileNumber) : undefined;
      userObj['email'] =  (udyamData?.alternativeUsername) ? (udyamData?.alternativeUsername) : undefined;
    }
    else if (curJourney?.isETB) {
      userObj['name'] =
      curJourney.metaData.capturedData.CUSTOMER_NAME;
      userObj['gender'] =
        curJourney.metaData.capturedData.gender;       
      userObj['dob'] =
      curJourney.metaData.capturedData.DATE_OF_BIRTH;
    } 
    else if (curJourney.resumeJourney) {
      let borrowerDetail = curJourney?.borrowerDetails?.borrowerDetail;
      userObj['name'] = this.getFullName(
        borrowerDetail?.borrowerDetail?.firstName,
        borrowerDetail?.borrowerDetail?.middleName,
        borrowerDetail?.borrowerDetail?.lastName
      );
      userObj['gender'] = borrowerDetail?.borrowerDetail?.gender;
      userObj['dob'] = new Date(moment(borrowerDetail?.borrowerDetail?.dateOfBirth,'DD-MM-YYYY').format('YYYY/MM/DD')).toISOString(); 
    }
    try{
        Appice?.setUser(userObj);
    }catch(e){
