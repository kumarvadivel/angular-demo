...
export class SectionBuilderComponent implements OnInit {
  ...
  saveUpdateChecklistActivity(){
    ...
    if(journey.product.productCode == "KML" || journey.product.productCode == "TML"){
      let checkList = []
      for( let field of this.config[this.config.length - 1].sectionContent.fields){
        if(field.checkListData){
          let list = {
            uuid: field.uuid,
            desc: "",
            datePicker: "",
      ...
      journey = this.commonService.getJourney()
      journey.metaData.capturedData['JOURNEYPREVIEW']={}
      journey.metaData.capturedData['JOURNEYPREVIEW']['checkListData'] = checkList;
  ...
  callApiLayer(submitflow,currentIndex,totalLength,section,submitflowList){
    switch(submitflow.submitflow){
      ...
      break;
      case 'updateMultiFacility':
        this.theme1ApiService.updateFacilityDetails(submitflow.mappedParameters).then((res:any)=>{            
          this.apiResponseDirectionFormSubmit(submitflow,res,currentIndex,totalLength,section,submitflowList)
        })
      break;
      case 'updateChecklistActivity':
        this.theme1ApiService.updateChecklistActivity(submitflow.mappedParameters).then((res:any)=>{
          this.apiResponseDirectionFormSubmit(submitflow,res,currentIndex,totalLength,section,submitflowList)
        })
      break;
      case 'updateSecurityDetails':
        this.theme1ApiService.updateSecurityDetails(submitflow.mappedParameters).then((res:any)=>{
          this.apiResponseDirectionFormSubmit(submitflow,res,currentIndex,totalLength,section,submitflowList)
        })
      break;
      case 'updateSelfEmploymentDetails':
        this.theme1ApiService.updateSelfEmployment(submitflow.mappedParameters).then((res:any)=>{
          this.apiResponseDirectionFormSubmit(submitflow,res,currentIndex,totalLength,section,submitflowList)
        })
      break;
      case 'updateLoanApplication':
        this.theme1ApiService.updateLoanApplication(submitflow.mappedParameters).then((res:any)=>{
          this.apiResponseDirectionFormSubmit(submitflow,res,currentIndex,totalLength,section,submitflowList)
        })
      break;
      case 'companyProfileUpdate':
        this.theme1ApiService.companyProfileUpdate(submitflow.mappedParameters).then((res:any)=>{
          this.apiResponseDirectionFormSubmit(submitflow,res,currentIndex,totalLength,section,submitflowList)
        })
      break;
      case 'updateCompanyRepresentative':
        this.theme1ApiService.updateCompanyRepresentative(submitflow.mappedParameters).then((res:any)=>{
          this.apiResponseDirectionFormSubmit(submitflow,res,currentIndex,totalLength,section,submitflowList)
        })
      break;
      default:
    ...
    gridSectionBuilderEmitter($event,section){
      this.status.emit({"ok":true,TriggerSection:section.componentType,GridEvent:$event})
    }
}
