...
import { isNull, reject } from '@pmmy-app/shared/utils/utils';
  ...
  @Input() field: any
    ...
    generateMobileOTP(resendOtp?: boolean) {
        ...
        if (this.options.individualOrCompany) {
            params['individualOrCompany'] = this.options.individualOrCompany
        }
    ...
    verifyMobileOtpResposeValidate(res) {
        this.showloader = false
        if (res.code == '200') {
            this.validateTheOtpResponse(res);
            setTimeout(() => {
    ...
    validateTheOtpResponse(res){
        if (this.options.saveData !== false) {
            let journey = this.commonService.getJourney();
            if (this.options.dontSaveResponse !== true) {
        ...
        this.theme1ApiService.verifyAadharOtp(params).then((res: any) => {
            ...
            if (res.code == '200') {
                ...
                if(res.code=='500'){
                    ...
                    this.verifyAadharOtpRemainingAttempts();
                    this.clear()
                    this.sharedService.openSnackBar(res.message, 'success', res.statusCode);
                }
            }
    ...
    verifyAadharOtpRemainingAttempts() {
        if (this.otpAttempt > 0) {
            this.otpAttempt -= 1
        }
