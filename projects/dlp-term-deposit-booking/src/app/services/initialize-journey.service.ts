...
import { LocalStorage } from "@tlad-app/shared/helpers/localStorage";
import { isEqual } from "@tlad-app/shared/utils/utils";
...
export class InitializeJourneyService {
    ...
    mapMasterData() {
        this.theme1ApiService.fetchBranchList().then((res: any) => {
            if (res.statuscode == 200) {
                ...
                localStorage.setItem("MASTER", JSON.stringify(ob));
            }
        });
    }
    ...
    saveProductDataToLocalStorage(product) {
        localStorage.setItem("SELECTED_PRODUCT", JSON.stringify(product.id));
        localStorage.setItem("SELECTED_LOAN_PRODUCT", JSON.stringify(product));
    }
    ...
    setJourney(journey) {
        this.localStorage.setItem("CURRENT_JOURNEY", journey);
    }

    getJourney() {
        return this.localStorage.getItem("CURRENT_JOURNEY");
    }
    ...
    getLocalStoreProductConfiguration() {
        return this.localStorage.getItem("PRODUCT_CONFIGURATION");
    }
    ...
    appiceEvent(eventName, product) {
        ...
        if (JSON.parse(localStorage.getItem("utm_source"))) {
            prop["OpenTrigger"] = "SocialMedia";
        } else {
            let referrer = JSON.parse(localStorage.getItem("document_referer"));
            if (
