...
import { cloneDeep, findLastIndex } from '@tlad-app/shared/utils/utils';
...
export class MetaConfigService {
    ...
    fetchProductPageConfig(journey, pageCode) {
        ...
        if (this.commonVariableService.tenentConfiguration.FETCH_PRODUCT_CONFIG_FROM_JSON_FILE) {
            productConfig = this.localStorage.getItem('PRODUCT_CONFIGURATION')
    ...
    fetchProductMetaConfig(journey, pageCode) {
        if (this.commonVariableService.tenentConfiguration.FETCH_PRODUCT_CONFIG_FROM_JSON_FILE) {
            let productConfig = this.localStorage.getItem('PRODUCT_CONFIGURATION')
    ...
    ParseConfig(config) {
        ...
        config.forEach((conf, indx) => {
            ...
            if(conf.componentType == 'OTP'){
                conf.sectionContent = {...conf.sectionContent, verificationExternalTrigger: new Subject<void>()}
            }
        })
        return config
    }
