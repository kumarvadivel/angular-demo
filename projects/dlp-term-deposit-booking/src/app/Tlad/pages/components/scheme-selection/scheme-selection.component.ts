...
@Component({
  selector: 'app-scheme-selection',
  templateUrl: './scheme-selection.component.html',
  styleUrls: []
})
export class SchemeSelectionComponent implements OnInit {
  journey: any;
  tabsData: any;
  showloader = false;
  metaConfig;
  ...
  sectionBuilderEmitter($event){
    if(this.journey.product.productCode=='LAAODTDR'){
      if($event.TriggerSection=='BUTTON'){
        if($event.data == 'SCHEME_SELECTION_SUBMIT'){
          this.submitSchemeSelection()
        }
      }
    }
  }
  ...
  submitSchemeSelection(){
    //validate the selection and process the submit
    this.formSubmitFlow()
  }
}
