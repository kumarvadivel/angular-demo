...
export class DynamicFieldsComponent implements OnInit {
    ...
    runValidationJson(field, initialLoad?, skip_calculation?, customDynamicList?) {
        for (const specifiers in field.validationJson) {
            switch (specifiers) {
                ...
                case 'property':
                    ...
                    break;
                case 'apiEndpoint':
                    field = this.runValidationApiEndpoint(field, initialLoad, customDynamicList, skip_calculation);
                    break;
                case 'currentDate':
    ...
    runValidationApiEndpoint(field, initialLoad, customDynamicList, skip_calculation) {
    ...
    formulamapping(item, _emit?, initialLoad?, skip_calculation?, depenedentCall?, customDynamicList?) {
        ...
        if (item.validationJson != null) {
            if (item.dontTriggerSelfValidation !== true) {
                this.runValidationJson(item, initialLoad, skip_calculation, customDynamicList)
            } else {
        ...
        if (item.rulesFor != null && item.rulesFor.length != 0) {
            item.rulesFor.forEach(rule => {
                ...
                if (f != undefined) {
                    this.formulamapping(f, true, initialLoad, skip_calculation, true, customDynamicList)
                }
            })
        }
    }
    ...
    rulesForMapping(field) {
        ...
        if (field.rulesFor != null && field.rulesFor.length != 0) {
            field.rulesFor.forEach(fieldName => {
                ...
                if (f != undefined) {
                    this.formulamapping(f, false, true)
                }
            })
        }
        return field
    ...
    processRegularField(field, data) {
        ...
        switch (field.fieldDataType) {
            ...
            case 'AUTO_COMPLETE':
                ...
                data[field.fieldName] = option?.optionKey || (field.searchoption ? null : field.value);
    ...
    mapTableCommonProperty(tblFields) {
        if (tblFields.fieldDataType == 'DROPDOWN' && tblFields.commonpropertyType != undefined && tblFields.commonpropertyType != null) {
            if (this.commonVariableService.static_commonProperty_KeyCodes.includes(tblFields.commonpropertyType)) {
                let opti = this.localStorage.getItem(tblFields.commonpropertyType)
