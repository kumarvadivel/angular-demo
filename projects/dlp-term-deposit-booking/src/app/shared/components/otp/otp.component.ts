...
import { isNull, reject } from '@tlad-app/shared/utils/utils';
...
export class OtpComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit(): void {
        this.verificationExternalTrigger?.subscribe((_data: any) => {
            this.validateOtp()
        })
        ...
        this.loanProduct = JSON.parse(localStorage.getItem('SELECTED_LOAN_PRODUCT'));
    }
    ...
    handleFillEvent(value) {
        ...
        if (this.otpGate) {
            if(!this.options.externalTriggerValidate){
                this.validateOtp();
            }
        }
