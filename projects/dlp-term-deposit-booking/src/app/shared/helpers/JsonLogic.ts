...
export class LogicFunctions {
    ...
    masterValue(masterkeyName) {
        let capturedData = document['jsonLogicRef'].localStorage.getItem('CURRENT_JOURNEY').metaData.capturedData
    ...
    subFlowValue(masterkeyName) {
        let capturedData = document['jsonLogicRef'].localStorage.getItem('SUBFLOW').capturedData
    ...
    JourneyValue(path) {
        let current = document['jsonLogicRef'].localStorage.getItem('CURRENT_JOURNEY');
    ...
    appendPathFetch(path) {
        ...
        path.map(sp => {
            if (sp.includes('[STATIC]')) {
                ...
                let curr = document['jsonLogicRef'].localStorage.getItem('CURRENT_JOURNEY');
                sp.split('.').forEach((p) => {
                    if (curr[p] != undefined) {
                        if (curr[p] || curr[p] == "") {
                            curr = curr[p];
    ...
    findMasterObject(Mastercode, searchParam, searchValue, returnKey) {
        let data = document['jsonLogicRef'].localStorage.getItem('MASTER')[Mastercode]
    ...
    getResume(_resumeJourney) {
        let journey = document['jsonLogicRef'].localStorage.getItem('CURRENT_JOURNEY')
