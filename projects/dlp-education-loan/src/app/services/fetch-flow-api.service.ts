...
export class FetchFlowApiService {
  ...
  fetchFlowApiCalls(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode) {
    ...
    } else if(fetchflow.fetchflow == "multiFacilityRetreive"){
    ...
    } else if(fetchflow.fetchflow == "fetchEligibilityDetails"){
    ...
    } else if(fetchflow.fetchflow == "fetchMultiFacilityList"){
    ...
    }else if(fetchflow.fetchflow == "retrieve"){
      this.theme1ApiService.retrieve(fetchflow.mappedParameters).then((res:any)=>{
        let journey =  this.getJourney()
        for (let list of res.companyRepresentativeList) {
          if (list.keyPerson == true) {
            journey.metaData.capturedData.keyPersonDetails = list
            list.firstName == undefined ? list.firstName = null : ""
            list.directorTextVariable23 == undefined ? list.directorTextVariable23 = null : ""
            list.lastName == undefined ? list.lastName = null : ""
  ...
  private fetchflowResponseDirection({
    ...
    this.fetchflowResponseValidator({
      ...
      response,
      pageCode,
      lastIndex: currentIndex != totalLength - 1 ? false : true
    })
  }
