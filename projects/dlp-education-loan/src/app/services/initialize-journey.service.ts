...
export class InitializeJourneyService {
    ...
    mapProductData(journey, dontRoute, reesumeJourney?) {
        this.theme1ApiService.fetchLoanProducts().then((res: any) => {
            let product;
            if (journey.product.productCode == "EDL") {
                product = res.loanProductList.find((f) => f.loanPurposeCode == "EDL");
    ...
    mapResumeData() {
        let resumeData = JSON.parse(
            sessionStorage.getItem("RESUME_APPLICATION_DATA")
        );
    ...
    updateStepCompletion(stepData: any, nextStepData: any): { updatedStepData: any } {
        if (stepData.subStep) {
            ...
            let stepperCompletion = stepData.subStep.filter(e => !e.isCompleted)
