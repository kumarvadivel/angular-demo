...
import { LogicFunctions } from '@el-app/shared/helpers/JsonLogic';
import { Formatters } from '@el-app/shared/helpers/Formatters';
...
export class DynamicFieldsComponent implements OnInit {
  ...
  renderCardOption(option) {
      return `../../../assets/images/feedback/${option.icon}.png`
  }
  ...
  checkTableFilled(dataField, index?) {
      ...
      if (result == true) {
          ...
          let arr = JSON.parse(JSON.stringify(fielddata.filter(e => e.ok == true)))
  ...
  tableActionDelete(_action, _tableRow, field, index) {
      _tableRow[0].value == "Term Loan" ? field.addMoreForTl = true : _tableRow[0].value == "Cash Credit" ? field.addMoreForCc = true : ""
  ...
  checkProposedLimit(field, index){
      if(field.value[index][0].value == "Term Loan"){
          ...
          if(enterLimit.value <= this.options.externalFeedDataforValidationJson.fetchEligibilityDetails.output.assessmentEligibility.tlEligibilityRound){
              ...
              this.sharedService.openSnackBar('Cash Credit proposed amount should not be greater than Cash Credit eligiblity amount ' + this.options.externalFeedDataforValidationJson.fetchEligibilityDetails.output.assessmentEligibility.tlEligibilityRound, 'error', 200);
              return false
          }
      }
  ...
  tableActionAccept(action, tableRow, field, index) {
      if(field.fieldName == "facilityDetailsListTable"){
          if( this.checkProposedLimit(field, index)){
