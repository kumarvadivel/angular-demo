...
import { isNull, reject } from '@tl-app/shared/utils/utils';
...
export class OtpComponent implements OnInit, AfterViewInit {
    ...
    generateMobileOTP(resendOtp?: boolean) {
        ...
        this.theme1ApiService.generateMobileOtp(params)
            .then((res: any) => {
                this.apiresponseHandler(res, resendOtp,"MOBILE")
            })
            .catch(error => {
    ...
    apiresponseHandler(res, resendOtp,verificationType) {
        if (res.code === '200') {
            this.handleSuccess(res, resendOtp,verificationType);
        } else {
    ...
    generateEmailOTP(resendOtp?: boolean) {
        ...
        this.theme1ApiService.generateEmailOtp(params)
            .then((res: any) => {
                this.apiresponseHandler(res, resendOtp,"EMAIL")
    ...
    generateAccountOTP() {
        ...
        this.theme1ApiService.generateAadharOtp(params)
            .then((res: any) => {
                if (res.code === '200') {
                    this.handleSuccess(res,params, false,"AADHAR");
                } else {
                    this.handleError(res, false);
                }
            })
    ...
    handleSuccess(res: any,params:any, resendOtp?: boolean,verificationType?:string) {
        this.showloader = false;
        let journey = this.commonService.getJourney();
        journey.metaData.capturedData = {...journey.metaData.capturedData, ...res};
        if(verificationType=="AADHAR" && params.requestFor == 'CO_APPLICANT'){
            journey.metaData.capturedData['coapplicantAadharData'] = res
        }
    ...
    mapValidateOtpParams(params) {
        switch (this.field.otpType) {
            ...
            case 'AADHAR':
                ...
                if(this.options.extraParams['coApplicantUuid']){
                    params['coApplicantUuid'] = this.options.extraParams['coApplicantUuid']
                }
                break;
        }
        ...
        this.theme1ApiService.verifyAadharOtp(params).then((res: any) => {
            ...
            if (res.code == '200') {
                if (this.options.saveToSubFlow) {
                    ...
                    if(params.requestFor == 'CO_APPLICANT')
                        this.journey.metaData.capturedData['coapplicantaadharData']  = res
