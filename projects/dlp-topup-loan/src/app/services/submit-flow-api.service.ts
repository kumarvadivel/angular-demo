...
handleErrorSubmitflow({ submitflow, currentIndex, totalLength, configList, pageconfig, pageCode, journey, _noLoader }: { submitflow; currentIndex; totalLength; configList; pageconfig; pageCode; journey; _noLoader; }){
     return (err) => {
        ...
        if (API_NAME_MAPPING[submitflow.submitflow]) {
            ...
            this.theme1ApiService[apiMethod](submitflow.mappedParameters)
                .then(
                    ...
                    this.handleErrorSubmitflow({ submitflow, currentIndex, totalLength, configList, pageconfig, pageCode, journey, _noLoader })
                );
        } else {
            let res
    ...
    mapFormSubmitErrorMessages(result, errorMessages) {
        if (result?.includes('[ERROR_MESSAGE]')) {
            ...
            if (splited.length > 1) {
                if(splited[1].includes('[ERROR_EVENTS]')){
                    let errorEvent = splited[1].split('[ERROR_EVENTS]')
                    splited[1] = errorEvent[0]
                    let data :any = {eventCode:errorEvent[1]}
                    this.commonVariableService.scopeSubject.next(data)
                }
            ...
            switch (result) {
                ...
                case 'PROCEED':
                    if (lastIndex) {
                        ...
                        this.formSubmitEventExecutor(submitflow)
                        this.apiCall({ submitflow: configList.formSubmitFlow[currentIndex + 1], currentIndex: currentIndex + 1, totalLength, pageCode, configList, pageconfig, extraCloudParams, _noLoader })
                    }
                    break;
                case 'ERROR':
    ...
    apiresponseValidator({
        ...
        if (currentIndex != totalLength - 1) {
            if (submitflow.validateResponse) {
                ...
                this.formSubmitEventExecutor(submitflow)
                if (journeyPage) {
                    this.apiCallJourneyPage({ submitflow: configList.formSubmitFlow[currentIndex + 1], currentIndex: currentIndex + 1, totalLength, pageCode, configList, pageconfig, extraCloudParams, _noLoader }) 
                } else {
                    this.apiCall({ submitflow: configList.formSubmitFlow[currentIndex + 1], currentIndex: currentIndex + 1, totalLength, pageCode, configList, pageconfig, extraCloudParams, _noLoader })
                }
            }
    ...
    apiCallJourneyPage({ submitflow, currentIndex, totalLength, pageCode, configList, pageconfig, extraCloudParams, _noLoader }: { submitflow; currentIndex; totalLength; pageCode; configList; pageconfig?; extraCloudParams?; _noLoader?; }) {
    ...
    handleProceedAction(submitflow, currentIndex, totalLength, pageCode, configList, pageconfig) {
        this.formSubmitEventExecutor(submitflow);
        this.apiCall({ submitflow: configList.formSubmitFlow[currentIndex + 1], currentIndex: currentIndex + 1, totalLength, pageCode, configList, pageconfig });
    }
    ...
    apiCall({ submitflow, currentIndex, totalLength, pageCode, configList, pageconfig, extraCloudParams, _noLoader }: { submitflow; currentIndex; totalLength; pageCode; configList; pageconfig?; extraCloudParams?; _noLoader?; }) {
        if(_noLoader==undefined){
