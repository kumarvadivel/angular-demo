...
import { ViewDocumentPopupComponent } from '@tl-app/shared/components/view-document-popup/view-document-popup.component';
...
@Component({
  selector: 'app-co-applicant-document-upload',
  templateUrl: './co-applicant-document-upload.component.html',
  styleUrls: ['./co-applicant-document-upload.component.scss']
})
export class CoApplicantDocumentUploadComponent implements OnInit {
  journey;
  showloader = false
  tabsData: any;
  metaConfig;
  ...
  configContentModerationWithRespectToProduct() {
      if (this.journey.product.productCode == 'PTLTP') {
          this.metaConfig.documentList.forEach(documentObj => {
              documentObj.showDocument = this.commonService.applyJsonLogic(documentObj.showDocument, {...this.$scope,...this.journey})
          })
  ...
  uploadDocument(option, documentObj) {
      if (option.disabled !== true) {
          let params = {
              ...
              coApplicantUuid:this.journey.metaData.capturedData.entityUuid,
  ...
  fetchDocumentStatus(documentObj, iterationCount) {
      ...
      this.theme1ApiService.checkDocumentUploadStatus(params).then((res: any) => {
          ...
          } else if (iterationCount != this.metaConfig.documentStatusCheckData.iteration) {
                  setTimeout(() => {
                      this.fetchDocumentStatus(documentObj, iterationCount + 1)
                  }, this.metaConfig.documentStatusCheckData.iterationDelay * 1000) 
          }else {
  ...
  edwLookupforPtlex(_document, notSubmit?) {
      ...
      let params = {
          ...
          "subStatusChangeDescription": `Coapplicant ${this.journey.metaData.capturedData.entityUuid} EDW Bank Statement`
      }
  ...
  edwDocumentStatusPinger(_document, iteration, notSubmit?) {
      ...
      this.theme1ApiService.getUploadedDoc(params).then((res: any) => {
          if(res.code=='200' || res.code==undefined){
              let doc = res.supportingDocuments.find(d => d.documentCategoryCode == _document.documentData.documentCategoryCode && _document.documentData.documentFor == d.DocumentFor && d.documentUploadedSuccessfully === true && d.CoapplicantUuid == this.journey.metaData.capturedData.entityUuid)
              if (doc != undefined) {
                  this.commonVariableService.globalLoaderState = false
                  this.commonVariableService.globalLoaderState = false
  ...
  readOrigin() {
      ...
      return getOrigin.includes('localhost') ? window.origin + "/#" : window.origin + '/top-up-loan' + "/#";
  }
  ...
  bsaFetch() {
    this.substatusChange("SUB_STATUS_31",`Coapplicant ${this.journey.metaData.capturedData.entityUuid} Banking statement fetch`)
  }
  ...
  uploadFieldDocument(documentObj) {
      ...
      this.theme1ApiService.uploadDocumentFile(params).then((res: any) => {
          if (res.code == '200') {
              ...
              this.substatusChange("SUB_STATUS_30",`Coapplicant ${this.journey.metaData.capturedData.entityUuid} salary statement upload`)
