...
import { isEqual } from "@nsc-app/shared/utils/utils";
...
export class InitializeJourneyService {
  ...
  private mapProductData(journey, dontRoute, reesumeJourney?) {
    this.apiService.fetchLoanProducts().then((res: any) => {
      ...
      }else if (journey.product.productCode == "NSC") {
        product = res.loanProductList.find((f) => f.loanPurposeCode == "SBA");
        this.appiceInitializer(product);
      }else {
  ...
  navigateJourney(
    ...
    if(pageCode == 'EKYC_VERIFY'){
      redirectUrl = redirectUrl + "?isDedupe=true";
    }
    if (urlParams) {
