...
import appConfiguration from '@nsc-app/app.config.json'
...
export class ApiService {
  ...
  UPLOAD_DOCUMENT_FILE=`api-v3borrower/uploadDocument`;
 ...
 BRANCH_STATE_MASTER =`/api-v3/restPublic/master-branch-list-new`;
  ...
  verifyAccNum(params) {
    ...
    let encrypted = this.encryptFormBodyData_JSON(body)
    return this.commonApiService.doPostApiCall(this.BASE_URL_MICROSERVICE + this.VERIFY_ACCOUNT_NUM, encrypted.body, undefined, undefined, {
      'Content-Type': 'application/json',
      'clientApiKey': encrypted.plainiv
    })
  }
