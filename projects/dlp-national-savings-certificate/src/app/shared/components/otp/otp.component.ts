...
import { JourneyEventsService } from '@nsc-app/services/journey-events.service';
import { InitializeJourneyService } from '@nsc-app/services/initialize-journey.service';
import { find, isEqual, isNull, reject } from '@nsc-app/shared/utils/utils';
...
export class OtpComponent implements OnInit, AfterViewInit {
  ...
  mapMobileValidateOtpParams(params) {
    ...
    if (this.journey.metaData.globalScopeData["redirectUrlData"]) {
      params['crmLeadId'] = this.journey.metaData.globalScopeData["redirectUrlData"]["crmLeadId"];
  ...
  resumeApplication() {
    ...
    if (pageCodeData) {
      if(journey.resumeJourney.isAccountNumberVerified == false && journey.resumeJourney.status == "INITIALIZED" && journey.resumeJourney.subStatus == "SUB_STATUS_4" && journey.resumeJourney.isETB == true){
