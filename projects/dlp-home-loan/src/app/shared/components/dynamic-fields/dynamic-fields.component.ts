...
import { FormValidationService } from '@hl-app/services/form-validation.service';
...
export class DynamicFieldsComponent implements OnInit {
          ...
          group = group.map(row => {
           row =  row.map(field => {
              if(field.fieldDataType!='ADDRESS'){
                field = this.HardCodeValues(field,fieldNames_alwaysRun,fieldNames_dontTrigger)
              }else{
                field.addressFields = field.addressFields.map(addressField=>{
                    addressField = this.HardCodeValues(addressField,fieldNames_alwaysRun,fieldNames_dontTrigger)
                    return addressField
    ...
    PremapchildOption_v1(data, option) {
        this.dynamicGroupList.forEach((group) => {
            group.forEach((row) => {
                row.forEach((e) => {
                    if (e.fieldDataType == 'DROPDOWN' || e.fieldDataType == 'AUTO_COMPLETE') {
                        e=this.premapchildOption_v1CommonMapper(e, data, option)
                    }
                    if (e.fieldDataType == 'ADDRESS') {
                        e.addressFields.forEach(addressField => {
                            if (addressField.fieldDataType == 'DROPDOWN' || addressField.fieldDataType == 'AUTO_COMPLETE') {
                                 this.premapchildOption_v1CommonMapper(addressField, data, option)
                            }
            ...
            group.forEach((row) => {
                row.forEach((e) => {
                    ...
                    if (e.fieldDataType == 'ADDRESS') {
                        e.addressFields.forEach(addressField => {
                            if (addressField.fieldDataType == 'DROPDOWN' || addressField.fieldDataType == 'AUTO_COMPLETE') {
                                this.resetOptions_v2CommonMapper(e, data)
                            }
                        })
                    }
                })
    ...
    mapchildOption_v1CommonMapper(e, data, option, valueresetted) {
        if (e.dependentField == data.fieldName) {
            e['optionsnapShot'] = option==undefined?[]:e.options?.filter(f => f.optionParentCode == option.optionCode)
            e['searchoption'] = e['optionsnapShot']
            e['noOptions'] = e['searchoption'].length == 0
            ...
            group.forEach((row) => {
                row.forEach((e) => {
                    ...
                    if (e.fieldDataType == 'ADDRESS') {
                        e.addressFields.forEach(addressField => {
                            if (addressField.fieldDataType == 'DROPDOWN' || addressField.fieldDataType == 'AUTO_COMPLETE') {
                                this.mapchildOption_v1CommonMapper(addressField, data, option, valueresetted)
                            }
    ...
    getchildLabel(data, dependentField, parent): any {
        data.forEach((group) => {
            group.forEach((row) => {
                row.forEach((e) => {
                    if (e.fieldDataType == 'DROPDOWN' || e.fieldDataType == 'AUTO_COMPLETE') {
                        this.getChildLabel(e, dependentField, parent)
                    }
                    if (e.fieldDataType == 'ADDRESS') {
                         e.addressFields.forEach(addressField => {
                            if (addressField.fieldDataType == 'DROPDOWN' || addressField.fieldDataType == 'AUTO_COMPLETE') {
                                this.getChildLabel(addressField, dependentField, parent)
                            }
    ...
    runValidationJson(field, initialLoad?, skip_calculation?, customDynamicList?,initialLoadRulesForcall?) {
        for (const specifiers in field.validationJson) {
            switch (specifiers) {
                ...
                case 'property':
                    ...
                    break;
                case 'apiEndpoint':
                    field = this.runValidationApiEndpoint(field, initialLoad, customDynamicList, skip_calculation,initialLoadRulesForcall);
                    break;
                case 'currentDate':
    ...
    runValidationProperty(field, customDynamicList) {
        for (const property in field.validationJson.property) {
            ...
            field.jsonlogicData = this.fetchlogicvariableData(
                customDynamicList || this.dynamicGroupList,
                field.logicvariables
            );
    ...
    runValidationApiEndpoint(field, initialLoad, customDynamicList, skip_calculation,_initialLoadRulesForcall) {        field.logicvariables = this.fetchLogicVaraiables(field.validationJson.apiEndpoint);
        field.jsonlogicData = this.fetchlogicvariableData(
            customDynamicList || this.dynamicGroupList,
            field.logicvariables
        );
    ...
    mapFormForEdit_vEndpointLogic(fieldsListgroup,source,errors,initialLoad?,skip_calculation?,endpoinOrigin?){
        fieldsListgroup.forEach(group => {
          group.forEach(row => {
            row.map(field => {
              ...
              this.mapErrorsForEndpointLogic(errors,field,initialLoad,skip_calculation)
      ...
      mapDataForDD(source,field,initialLoad,skip_calculation){
        if(typeof source[field["fieldName"]]=='object'){
           ...
           }else if(field['options'].some(e=>e.optionName==source[field["fieldName"]])){
                 field["value"] = source[field["fieldName"]]
        ...
        data.forEach((group) => {
            group.forEach((row) => {
                row.forEach((field) => {
                    ...
                    [mandatoryData, nonMandatoyData] = this.formValidationService.parseMandatoryCheckOnlyForShowFieldData(field, mandatoryData, nonMandatoyData, temp)
                });
            });
        });
          ...
          group.forEach((row) => {
            row.forEach((e) => {
                  if (e.showField===true && e.fieldDataType == 'ADDRESS' && e.isMandatory === false) {
                    ...
                    addresssubFieldStates = this.formValidationService.getAddressSubFieldStates(e)
                   
                    //find out the sub form is touched or not
                    temp = this.formValidationService.mandatorySubFormMandatory(addresssubFieldStates,temp)
    ...
    mapFormForEditAddress(e) {
        e.addressFields = e.addressFields.map(addressField => {
            if (this.options.mappingFields[e.fieldName].hasOwnProperty(addressField["fieldName"])) {
                ...
                if (this.options.mapAndDisable === true && addressField.value != null && addressField.value != "") {
    ...
    mapFormForEditTable(e) {
        ...
        e.rowActionData.forEach(da => {
            da.actions.forEach(dt => {
                if (dt.actionCode == "EDIT" || dt.actionCode == "ACCEPT") dt.isDisplay = true
            })
        })
            ...
            group.forEach((row) => {
                row.forEach((e) => {
                    ...
                    if (e.verificationFieldName && e.verificationFieldName == otpField.fieldName) {
                        ...
                        e.isVerified = true
                        e.fieldAccessModifier = 'READ_ONLY'
                        this.otherEvents.emit({"eventType": "verificationSuccess", "field": e, "eventName": "VERIFICATION_SUCCESS"})
    ...
    verifyMobileField(field){
        //abstraction : as method implements same functionalities
        this.verifyEmailField(field)
    }
