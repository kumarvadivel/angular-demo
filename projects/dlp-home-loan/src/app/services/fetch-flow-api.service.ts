...
export class FetchFlowApiService {
    ...
    fetchFlowApiCalls(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode) {
        ...
        } else if(fetchflow.fetchflow=='fetchSingleCoapplicant'){
                this.theme1ApiService.fetchCoApplicant(fetchflow.mappedParameters).then((res:any)=>{
                    if(res.code=='200'){
                        res = {
                            "code":res.code,
                            ...res.coApplicantList.find(s=>s.GUARANTOR_DETAIL.coApplicantUuid== fetchflow.dataCloud.capturedData.entityUuid)
                        }
                    }
                    this.fetchflowResponseDirection({
                        fetchflow,
                    ...
                    this.ErrrorFetchflowHandler({ fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, error: err, pageCode })
                })
            }else{
    ...
    ErrrorFetchflowHandler({ fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, error, pageCode }: { fetchflow; totalLength; currentIndex; scope; fetchflowList; pageconfig; error; pageCode; }){
