...
export class SubmitFlowApiService {
  ...
  private submitFlowApiCalls({
    ...
    const API_NAME_MAPPING = {
      ...
      'employmentProfileUpdate': 'employmentProfileUpdate',
      'CreateOrUpdateAddOnCardDetails': 'CreateOrUpdateAddOnCardDetails',
      'fetchEligibilityDtls': 'fetchEligilibilityDetails',
      'changeLoanApplicationStatus': 'updateMainLoanApplicationStatus',
      'minimumCheckParameter': 'fetchCibilData',
      'updateLoanProgram': 'updateLoanProgram',
      'updateNameOnCard': 'updateNameOnCard',
      'updatesubStatus': 'updateLoanApplicationStatus',
      'feedbackSubmit': 'feedbackSubmit',
      'fetchAtosCardDetails': 'fetchAtosCardDetails',
    ...
    if (API_NAME_MAPPING[submitflow.submitflow]) {
      ...
      this.apiService[apiMethod](submitflow.mappedParameters).then((res) => {
      ...
      let res;
      if (submitflow.submitflow === 'checkKeyfactStatus') {
        res = {};
        this.apiResponseDirectionFormSubmit({
          submitflow,
          res,
        ...
        console.error(
          `Your API [ ${submitflow.submitflow} ]  is not in the API_NAME_MAPPING list, please add`
        );
      }
    }
  }
  ...
  private apiCallJourneyPage(
    ...
    submitflow.dataCloud = this.fetchFlowApiService.fetchDataCloud(
      submitflow,
      pageconfig
    );
    if (extraCloudParams) {
  ...
  apiCall(
    ...
    if (submitflow.validationJson) {
      let result = this.applyJsonLogic(
        submitflow.validationJson,
        submitflow.dataCloud
      );
      switch (result) {
        case "EXECUTE":
    ...
    if (submitflow.saveResponseToCapturedData) {
      switch (submitflow.submitflow) {
        ...
        case 'fetchAccountData':
          journey.metaData.capturedData = { ...journey.metaData.capturedData, AccountData: response.object }
          break;
