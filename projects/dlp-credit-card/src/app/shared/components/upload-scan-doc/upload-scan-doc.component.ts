...
export class UploadScanDocComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<UploadScanDocComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any, public commonService: CommonCommonService, public apiService: ApiService,
    public commonVariableService: CommonVariableService, public sharedService: SharedService, private localStorage: LocalStorage) { }
  ...
  ngOnInit(): void {
    ...
    if (this.data?.type == 'fields_with_upload') {
      ...
      for (let selectedDocData of this.data.selectedDocumentData.alias) {
        this.commonProperty_static.BANK_NAMES.push({
          'optionName': selectedDocData.name,
          'optionKey': selectedDocData.uuid,
          'optionCode': selectedDocData.code
        })
      }
  ...
  completeTransaction() {
    ...
    if (this.data.parentObj.documentName == '6 Months Bank Statement' && this.data.childObj.category == 'scannedStatementUpload') {
      ...
      this.apiService.completeTransact(params, url).then((res: any) => {
        if (res.code == 200) {
