...
export class LogicFunctions {
    ...
    constructor(public ArrayMethods: ArrayMethod,
        ...
        public commonVariableService: CommonVariableService, private localStorage: LocalStorage) {
        this.init()
    }
    ...
    init() {
        ...
        this.Jsonlogic.add_operation('sumOfNetWeight', this.sumOfNetWeight)
    ...
    masterValue(masterkeyName) {
        let capturedData = this.localStorage.SessionGetItem('CURRENT_JOURNEY').metaData.capturedData
    ...
    subFlowValue(masterkeyName) {
        let capturedData = this.localStorage.SessionGetItem('SUBFLOW').capturedData
    ...
    findMasterObject(Mastercode, searchParam, searchValue, returnKey) {
        let data = this.localStorage.SessionGetItem('MASTER')[Mastercode]
    ...
    //this method will return the parameter given url parameter value if the parameter is not present then it will return null
    ...
    sumOfNetWeight(value) {
        values.coinValue += value;
    }
    ...
    currencyFormatter(value) {
        if (value !== '') {
            ...
            if (!isNaN(numberValue)) {
                ...
                symbolized = new Intl.NumberFormat('en-IN', formatOptions).format(numberValue);
            } else {
                symbolized = null;
            }
            return symbolized;
