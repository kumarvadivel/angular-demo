...
export class LandingComponent implements OnInit {
  ...
  ngOnInit(): void {
      // this.localStorage.removeItem('PRODUCT_CONFIGURATION')
  ...
  navigatePage(item) {
    if(item.isActive){
        sessionStorage.setItem("SELECTED_PRODUCT",JSON.stringify(item.id))
        sessionStorage.setItem("SELECTED_LOAN_PRODUCT",JSON.stringify(item))
        sessionStorage.setItem("selectedProduct",JSON.stringify(item.id))
       window.open("/#/"+item.url,"_self")
    }else{
        this.snackbarService.openSnackBar('Stay Tuned !', 'success', 200);
    }
  }
