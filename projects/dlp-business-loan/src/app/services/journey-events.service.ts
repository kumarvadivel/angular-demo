...
export class JourneyEventsService {
  ...
  executeJourneyEvents_subMethod16(externalData, eventsObj, prop) {
    let mappingparams = {};
    if (externalData) {
      this.executeJourneyEvents_subMethod16Properties(eventsObj, prop, mappingparams, externalData);
      try {
        if(typeof Appice != 'undefined'){
          Appice.recordEvent({
            key: eventsObj[prop].key,
  ...
  executeJourneyEvents_subMethod16Properties(eventsObj, prop, mappingparams, externalData) {
    for (const k in eventsObj[prop]["properties"]) {
      if (Array.isArray(eventsObj[prop]['properties'][k])) {
        if (eventsObj[prop]['properties'][k][0] == "LOGIC") {
          mappingparams[k] = this.applyJsonLogic(eventsObj[prop]['properties'][k][1], externalData)
        } else {
          mappingparams[k] = this.ArrayMethods.getPathVal(externalData, this.applyJsonLogic(eventsObj[prop]['properties'][k][0], externalData))
        }
      } else {
        mappingparams[k] = eventsObj[prop]["properties"][k];
      }
