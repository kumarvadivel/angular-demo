...
export class DynamicFieldsComponent implements OnInit {
  ...
  updateRangeValue(field){
    ...
    mapAutoCompleteValueSubMethod(e, addressField) {
    ...
    mapAutoCompleteValue(e) {
        e.addressFields.forEach(addressField => {
            if ((addressField.fieldDataType == 'DROPDOWN' || addressField.fieldDataType == 'AUTO_COMPLETE')) {
                addressField = this.mapAutoCompleteValueSubMethod(e, addressField);
            ...
            if (this.options.mappingFields?.hasOwnProperty(addressField["fieldName"])) {
                ...
                if (addressField.fieldDataType == 'DATE') {
                    if (addressField.value != "" && addressField.value != null) {
                        addressField = this.mapFormForEditAddresFieldDate(addressField);
                    }
                }
    ...
    mapFormForEditAddresFieldDate(addField) {
        if (typeof addField.value == 'string') {
            let formattedDate = this.formatters.formatDate(addField.value, "DD/MM/YYYY", "MM/DD/YYYY")
            addField.value = formattedDate
            addField.value = new Date(addField.value)
        } else {
            if (addField.verificationType && this.options.autoVerifyMappedFields) {
                addField.isVerified = true
            }
        }
    ...
    async mapGstSearch(e) {
        ...
        if (e.commonpropertyType == 'GST_SEARCH_V3') {
            let params = {
                "access_token": this.journey.product.access_token,
                "loanUuid": this.journey.product.loanUuid,
                "microservicetoken": this.journey.product.oauthAccessToken
            }
            await this.theme1ApiService.getGstData(params).then((res: any) => {
                if (res.code == '200') {
                    e.suggestion = res.mappingFields.textVariable5.options
                }
    ...
    async gstSearchDropdownTypeV3(e) {
        let params = {
            "access_token": this.journey.product.access_token,
            "loanUuid": this.journey.product.loanUuid,
            "microservicetoken": this.journey.product.oauthAccessToken
        ...
        await this.theme1ApiService.getGstData(params).then((res: any) => {
            if (res.code == '200') {
                e.options = res.mappingFields.textVariable5.options;
                this.dynamicGroupList = this.copySearchArray(this.dynamicGroupList)
            }
        })
    }
            ...
            if (this.commonVariableService.static_commonProperty_KeyCodes.includes(e.commonpropertyType)) {
                switch (e.commonpropertyType) {
                    ...
                    case 'GST_SEARCH':
                        ...
                        break;
                    case 'GST_SEARCH_V3':
                        e = this.gstSearchDropdownTypeV3(e)
                        break;
    ...
    callAutoSuggestionApi(field) {
        ...
        switch (field.autosuggestCode) {
            ...
            case "GST_SEARCH_V3":
                ...
                this.theme1ApiService.getGstData(params).then((res: any) => {
                    if (res.code == '200') {
                        field.suggestion = res.mappingFields.textVariable5.options
                    }
                })
                break;
        }
