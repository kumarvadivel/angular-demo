...
import { isEqual } from "@kcc-ah-app/shared/utils/utils";
...
export class InitializeJourneyService {
    ...
    updateStepperDataFromJsonFile(pageObj, nextPageCode, journey, redirectPageObj, pageCode, redirectUrl) {
        ...
        if (journeyPages.findIndex(f => f.pageCode == nextPageCode) == -1) {
            pageObj = otherPages[otherPages.findIndex(f => f.pageCode == nextPageCode)]
        } else {
            pageObj = journeyPages[journeyPages.findIndex(f => f.pageCode == nextPageCode)]
        }
    ...
    steppUpdaterforPageSequnece(nextPageCode, pageCode, journey, skipped, _currentPage) {
        ...
        if (this.router.url.includes('isPreviwUpdate')) {
            ...
            let journeyOdj = this.getJourney();
            journeyOdj.isPreviewUpdate = false;
            this.setJourney(journeyOdj);
        } 
    ...
    updateStepper(journey, pageCode?, skipped?) {
        let i = 0
        if (pageCode) {
            let currentpageIndex = journey.journeyPages[journey.productUserType].findIndex(el => el.pageCode == pageCode)
            if (currentpageIndex != -1) {
                if (!journey.journeyPages[journey.productUserType][currentpageIndex].lastPage) {
                    let nextPageData = journey.journeyPages[journey.productUserType][currentpageIndex + 1]
                    journey.metaData.stepperData.forEach(stepData => {
                        if (stepData.isActive) {
                            this.updateStepperSubStep(stepData, nextPageData, pageCode, skipped, journey, i)
                        }
                        i++;
                    })
                }
    ...
    updateStepperSubStep(stepData, nextPageData, pageCode, skipped, journey, i) {
        if (stepData.subStep) {
            this.updateStepperSubStepSection(stepData, nextPageData, pageCode, skipped)
        }
        else if (stepData.pageCode) {
            stepData.isActive = true
            this.updateStepperPageCode(journey, nextPageData, pageCode, skipped, stepData, i)
            if (i >= journey.metaData.stepperData.length) {
                journey.metaData.stepperData[i + 1].isActive = true
            }

        }
        if (stepData.subStep) {
            let stepperCompletion = stepData.subStep.filter(e => e.isCompleted === false)
            if (stepperCompletion.length == 0) {
                if (journey.metaData.stepperData[i + 1] != undefined) {
                    journey.metaData.stepperData[i + 1].isActive = true
                }
    ...
    updateStepperSubStepSection(stepData, nextPageData, pageCode, skipped){
        stepData.subStep.forEach(subData => {
            ...
            if (subData.pageCode == pageCode) {
                if (skipped) {
                    subData.isCompleted = false;
                } else subData.isCompleted = true;
            }
        })
    }

    updateStepperPageCode(journey, nextPageData, pageCode, skipped, stepData, i){
        if (journey.metaData.stepperData[i + 1]?.subStep) {
            journey.metaData.stepperData[i + 1].subStep.forEach(subStep => {
                if (subStep.pageCode === nextPageData.pageCode) {
                    subStep.isActive = true
                    journey.metaData.stepperData[i + 1].isActive = true
                }
                if (subStep.pageCode == pageCode) {
                    if (skipped) {
                        subStep.isCompleted = false;
                    } else subStep.isCompleted = true;
                }
            })
        } else if (journey.metaData.stepperData[i + 1]?.pageCode == nextPageData.pageCode) {
            journey.metaData.stepperData[i + 1].isActive = true
        }
        if (pageCode == stepData.pageCode) {
            stepData.isCompleted = true
        }
    }
    ...
    private appiceInitializer(product) {
        ...
        if (this.journey.product.productCode == 'MSMERE') {
            this.appiceEvent("MSMEVisit", product);
        }
    }
}