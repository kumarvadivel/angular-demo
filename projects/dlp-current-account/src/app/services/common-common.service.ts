...
import { assign, uniqBy } from "@ca-app/shared/utils/utils";
...
export class CommonCommonService {
    ...
    getDocumentList(field?){
        ...
        return new Promise((resolve, _reject) => {
          this.theme1ApiService.getUploadedDoc(param).then((resp:any)=>{
            if(resp){
              resolve(resp)
                    ...
                    for (const prop in metaConfig.prepopulationRemaps) {
                        if (Array.isArray(metaConfig.prepopulationRemaps[prop])) {
                            mapping_Data[prop] = this.ArrayMethods.getPathVal(
                                data,
                                this.applyJsonLogic(
                                    metaConfig.prepopulationRemaps[prop][0],
                                    data
                                )
                            );
    ...
    if(form.constitution!=undefined){
      switch(form.constitution){
        ...
        case 'Sole Proprietorship':
            if(journey.product.productCode=="CA") {
                this.updateJourneyType('Sole Proprietorship')
                this.updateConstitution(form.constitution)
            } else {
  ...
  applicationResume() {
    this.journey=this.getJourney()
    this.journey.metaData.capturedData["isTrackApplication"] = true;
