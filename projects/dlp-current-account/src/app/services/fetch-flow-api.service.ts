...
import { find } from '@ca-app/shared/utils/utils';
...
export class FetchFlowApiService {
  ...
  fetchDataScopeData(fetchflow,totalLength,currentIndex,scope,fetchflowList,pageconfig,pageCode){
    ...
    if(fetchflow.validate){
       ...
       this.handleNoAccessTokenScenario(fetchflow);
        break;
       }
    }else{
      this.fetchFlowApiCalls(fetchflow,totalLength,currentIndex,scope,fetchflowList,pageconfig,pageCode) 
...
handleNoAccessTokenScenario(fetchflow){
  let journey = this.localStorage.SessionGetItem('CURRENT_JOURNEY');
  let pageCodeData=find(journey.journeyPages.individual, { 'pageCode': "MOBILE_VERIFY"}); 
  let pageCodeDataUrl = pageCodeData?.url;
  if(pageCodeData && pageCodeDataUrl){
    if(fetchflow.vkycFlow){
      this.router.navigateByUrl(pageCodeData.url+"?vkyc=true");
    }else{
      this.router.navigateByUrl(pageCodeData.url);
    }
  } 
}
  ...
  private fetchFlowApiCalls(fetchflow, totalLength, currentIndex, scope, fetchflowList, pageconfig, pageCode) {
    const API_NAME_MAPPING = {
      ...
      'fetchUserHierarchyDetails':'fetchUserHierarchyDetails',
      'fetchContracts':'fetchContractToAccept',
      'acceptContract':'acceptContract',
      'fetchLoanDetails':'fetchLoanDetails',
      'fetchNominee':'nomineeShow',
      'fetchVas':'showVas',
    ...
    if (API_NAME_MAPPING[fetchflow.fetchflow]) {
      if(fetchflow.fetchflow=='fetchVas'){
        fetchflow.mappedParameters['loanApplicationUuid']=fetchflow.mappedParameters['loanUuid'];
      }
      else if (fetchflow.fetchflow == "retrieve") {
        this.apiService.retrieve(fetchflow.mappedParameters).then((res: any) => {
          let journey = this.getJourney()
          for (let list of res.companyRepresentativeList) {
            if (list.keyPerson) {
              journey.metaData.capturedData.directorDetails = list
            }
          }
      ...
      this.apiService[apiMethod](fetchflow.mappedParameters).then((res: any) => {
