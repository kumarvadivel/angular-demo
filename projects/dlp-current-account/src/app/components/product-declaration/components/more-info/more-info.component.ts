...
export class MoreInfoComponent implements OnInit {
  ...
  bindMappingData(){
    if(this.journey?.isEtb){
      ...
      this.config[1].sectionContent.options['mapAndDisable']=false;
        ...
        this.config[1].sectionContent.options['mappingFields']=JSON.parse(JSON.stringify({...this.journey.metaData.capturedData.aadharData,...this.journey.metaData.capturedData.MOBILE_VERIFY
          ,...this.journey.metaData.capturedData.PAN_VERIFY,...this.journey.metaData.capturedData.borrowerDetails}));
      }
  ...
  sectionBuilderEmitter($event){ 
        ...
        if($event.TriggerSection=='FORM'){
          ...
          if ($event.formValue?.borrowerDetailTextVariable14 == 'NO') {
            this.sharedService.openSnackBar('This feature is currently available for Resident individual customers only, for opening non-resident account, please visit nearest branch', 'error', 200);
            setTimeout(() => {
            this.config[1].sectionContent.fields[9].value = "YES"
            }, 1000);
            }
        } 
  } 
