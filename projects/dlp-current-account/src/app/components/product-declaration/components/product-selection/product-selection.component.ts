...
export class ProductSelectionComponent implements OnInit {
  ...
  bindMappingData(){  
      ...
      this.loanProgramValue = sessionStorage.getItem("PRODUCT_SELECTION")
      } 
  ...
  continue() {
    ...
    if (!this.loanProgramValue) {
      ...
      sessionStorage.setItem("PRODUCT_SELECTION",this.loanProgramValue)
      this.showloader = true
      this.formSubmitFlow()
    }
  }
...
dataScopeFetchFlow(){
  if(this.metaConfig.dataScopeFetchFlow && this.metaConfig.dataScopeFetchFlow.length){
    this.showloader=true
    this.fetchFlowApiService.fetchDataScopeData(this.metaConfig.dataScopeFetchFlow[0],this.metaConfig.dataScopeFetchFlow.length,0,{},this.metaConfig.dataScopeFetchFlow,this.config,"PRODUCT_SELECTION")
  }
...
fetchLoanProgramData(){
  ...
  this.apiService.fetchLoanProgramData(params).then((res:any)=>{
    ...
    const indexOfBox = this.findIndexByProperty(this.loanprogramData, "uuid", sessionStorage.getItem("PRODUCT_SELECTION"))
    this.loanprogramData[indexOfBox]["selectedBox"] = true
  })
