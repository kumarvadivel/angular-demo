...
export class PersonalDetailsComponent implements OnInit {
  ...
  bindMappingData(){ 
      if(this.isPreview ){ 
        this.config[0].sectionContent.options['mapSupplyData']=true; 
        this.config[0].sectionContent.options['mapAndDisable']=false;
        this.config[0].sectionContent.options['mappingFields']={...this.journey.metaData.globalScopeData['fetchBorrowerDetails']['borrowerDetail']}
        this.config[1].sectionContent.options['mapSupplyData']=true; 
        this.config[1].sectionContent.options['mapAndDisable']=false;
        this.config[1].sectionContent.options['mappingFields']={...this.journey.metaData.globalScopeData['fetchNominee']['nomineeList'][0],...this.journey.metaData.globalScopeData['fetchNominee']['nomineeList'][0].addressOne,...this.journey.metaData.globalScopeData['fetchNominee']['nomineeList'][0].addressTwo}
      }else{
        this.config[1].sectionContent.options['mapSupplyData']=true; 
        this.config[1].sectionContent.options['mapAndDisable']=true;
        this.config[1].sectionContent.options['mappingFields']=JSON.parse(JSON.stringify({...this.journey.metaData.capturedData.aadharData,...this.journey.metaData.capturedData.borrowerDetails}));
        delete  this.config[1].sectionContent.options['mappingFields']['firstName'];
        delete  this.config[1].sectionContent.options['mappingFields']['dateOfBirth'];
        delete  this.config[1].sectionContent.options['mappingFields']['gender'];
      } 
}
  dataScopeFetchFlow(){
