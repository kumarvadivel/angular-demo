...
export class DocumentsUploadV2Component implements OnInit {
  ...
  journey;
    ...
    edwLookupforPtlex(document, notSubmit?) {
        this.commonVariableService.globalLoaderState = true
        let params = {
            access_token: this.journey.product.access_token,
            loanUuid: this.journey.product.loanUuid,
        ...
        this.edwUpdate(params, notSubmit, document)
    }

    edwUpdate(params, notSubmit, document) {
        this.theme1ApiService.updateLoanApplicationStatus(params).then((res: any) => {
            if (res.code == '200') {
                this.edwDocumentStatusPinger(document, 1, notSubmit)
            } else {
                this.commonVariableService.globalLoaderState = false
            }
        }, _err => {
    ...
    pingRecursive(iteration, document, notSubmit) {
        if (iteration != this.metaConfig.documentStatusCheckData.iteration) {
            setTimeout(() => {
                this.edwDocumentStatusPinger(document, iteration + 1, notSubmit)
            }, this.metaConfig.documentStatusCheckData.iterationDelay * 1000)
        } else {
            this.commonVariableService.globalLoaderState = false
            this.commonVariableService.globalLoaderState = false
    ...
    edwDocumentStatusPinger(document, iteration, notSubmit?) {
        let params = {
            access_token: this.journey.product.access_token,
            loanUuid: this.journey.product.loanUuid,
        ...
        this.theme1ApiService.getUploadedDoc(params).then((res: any) => {
            if(res.code=='200' || res.code==undefined){
                let doc = res.supportingDocuments.find(d => d.documentCategoryCode == document.documentData.documentCategoryCode && document.documentData.documentFor == d.DocumentFor && d.documentUploadedSuccessfully === true)
                if (doc != undefined) {
                    this.commonVariableService.globalLoaderState = false
                    this.commonVariableService.globalLoaderState = false
                    document.documentUploadStatus = 'COMPLETED'
                    document.documentUploadStatuslocalisation = 'Completed'
                    this.showShareSection = true
                    if (!notSubmit) {
                        this.formSubmitFlow()
                    }
                } else {
                this.pingRecursive(iteration, document, notSubmit)
                }
            } else {
                this.pingRecursive(iteration, document, notSubmit)
            }
                ...
                switch(doc.selectedBankData.value){
                    ...
                    case null:
                        if(doc.documentUploadStatus=='COMPLETED'){
                            ...
                            this.commonVariableService.globalLoaderState = false
                            this.showloader = false
                            this.sharedService.openSnackBar('Please Upload Mandatory Document')
                          }
                    break;
                }
    ...
    readOrigin() {
        ...
        return getOrigin.includes('localhost') ? window.origin + "/#" : window.origin + '/banking' + "/#";
    }
