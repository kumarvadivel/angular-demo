...
export class LogicFunctions {
    ...
    init() {
        ...
        this.Jsonlogic.add_operation('getVKYCLink',this.getVKYCLink)
        this.Jsonlogic.add_operation('checkIsNomineeCreatedAlready',this.checkIsNomineeCreatedAlready)
        ...
        this.Jsonlogic.add_operation('nomineeValue', this.nomineeValue)
    }
    ...
    //adding a numeric value to the Date 
    //params:Date= sourceDate , Date in milliseconds
    //addingValue = value to be added to the date,
    //addingMetric = metric in which the date need to be added; responded values ['DAYS','MONTHS','WEEKS','YEARS','QUARTERS','HALFYEARLY']
    ...
    truncate(number, truncate_stripoffValue?) {
        return parseFloat(number).toFixed(truncate_stripoffValue != undefined ? truncate_stripoffValue : 3)
    }
    ...
    masterValue(masterkeyName) {
        let capturedData =
          document["jsonLogicRef"].localStorage.SessionGetItem("CURRENT_JOURNEY").metaData.capturedData;
  ...
  JourneyValue(path) {
    let current =
      document["jsonLogicRef"].localStorage.SessionGetItem("CURRENT_JOURNEY");
  ...
  appendPathFetch(path) {
    ...
    getResume(_resumeJourney) {
        ...
        let pageCode = null;
        let param;
        [pageCode, param] = document["jsonLogicRef"].getPageCodeParam(
          MainpageSequence,
          [journey.resumeJourney.status, journey.resumeJourney.subStatus]
        );
        if (pageCode == null) {
          [pageCode, param] = document["jsonLogicRef"].getPageCodeParam(
            otherpageSequence,
    ...
    checkIsNomineeCreatedAlready(){
        let globalScopeData = document['jsonLogicRef'].localStorage.SessionGetItem('CURRENT_JOURNEY').metaData.globalScopeData;
        if(globalScopeData && globalScopeData['fetchNominee'] && globalScopeData['fetchNominee']['nomineeList'].length>0){
            return true
        }else{
            return false 
        }
    ...
    getVKYCLink(phone,crmLeadCode,loanPurposeUuid){ 
        let redirectUrl
        if(!window.origin.includes('localhost')){
           redirectUrl= `${window.origin}/current-account/#/vkyc/processing?encrypedParams=`+btoa(`mobileNumber=${phone}&crmLeadId=${crmLeadCode}&loanPurposeUuid=${loanPurposeUuid}`)
        }else{
            redirectUrl=  `${window.origin}/#/vkyc/processing?encrypedParams=`+btoa(`mobileNumber=${phone}&crmLeadId=${crmLeadCode}&loanPurposeUuid=${loanPurposeUuid}`)
        }
        localStorage.setItem("JOURNEY_SNAP",JSON.stringify(document['jsonLogicRef'].localStorage.SessionGetItem('CURRENT_JOURNEY')))
       return redirectUrl;
    }
    getLocalStorage(keyName){
      ...
      removeItemLs(keyName){
          ...
          //get data from the nominee value
    nomineeValue(masterkeyName) {
        let metaDataa =
          document["jsonLogicRef"].localStorage.SessionGetItem("CURRENT_JOURNEY");
        let nomineeValue = metaDataa;
        masterkeyName.split(".").forEach((p) => {
            if (nomineeValue?.[p]) {
                nomineeValue = nomineeValue[p];
            } else {
                nomineeValue = null;
            }
        });
