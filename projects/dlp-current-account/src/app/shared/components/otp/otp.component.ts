...
import { reject, isEqual,find,isNull } from '@ca-app/shared/utils/utils';
...
export class OtpComponent implements OnInit, AfterViewInit {
  ...
  generateMobileOTP(resendOtp?: boolean) {
    const params = {
      ...
      individualOrCompany: this.options.individualOrCompany == 'Individual' ? this.options.individualOrCompany.toUpperCase() : "COMPANY"
    };
    ...
    this.apiService.verifyEmailOtp(params).then(
      (res: any) => {
        ...
        if (res.code == "200") {
          ...
          this.journeyEventsService.journeyEventsExecutor("EMAIL_VERIFY_FAILED")
  ...
  verifyMobileOtp(params) {
    this.apiService.verifyMobileOtp(params).then(
      ...
      (error) => {
        this.showloader = false;
      this.journeyEventsService.journeyEventsExecutor("MOBILE_VERIFY_FAILED")
        this.sharedService.openSnackBar(error.message, "error", 400);
      }
    );
  }
    ...
    pageArr.map(pageseq => {
      if (pageseq.resumeJourneySequences) {
        pageseq.resumeJourneySequences.map(seq => {
          if (Array.isArray(seq) && seq.length >= 2 && isEqual(seq[0], sampleSeq[0]) && isEqual(seq[1], sampleSeq[1])) {
