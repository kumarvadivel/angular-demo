...
export class MoreInfoComponent implements OnInit {
  ...
  sectionBuilderEmitter($event) { 
    if ($event.TriggerSection == 'CONSENT') {
      this.handleConsentSection();
    } else if ($event.TriggerSection == 'FORM') {
      this.handleFormSection($event);
    }
  }

  handleConsentSection() {
  ...
  handleFormSection($event) {
    ...
    if ($event.formValue?.borrowerDetailTextVariable14 == 'NO') {
      ...
      setTimeout(() => {
        this.config[1].sectionContent.fields[10].value = "YES"
      }, 1000);
    }
  }
}
