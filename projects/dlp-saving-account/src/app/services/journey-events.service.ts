...
export class JourneyEventsService {
  ...
  private executeJourneyEvents_subMethod12(eventsObj, prop, externalData) {
    eventsObj[prop].map((d) => {
      ...
      if (externalData && typeof Appice != 'undefined') {
        if(d.validateKey){
          d.key =  this.applyJsonLogic(d.validateKey,externalData);
        }
        for (const k in d["properties"]) {
          if (Array.isArray(d["properties"][k])) {
            mappingparams[k] =  this.ArrayMethods.getPathVal(externalData,this.applyJsonLogic(d["properties"][k][0], externalData));
          } else if(typeof d["properties"][k] === 'object') {
            mappingparams[k] = this.applyJsonLogic(d["properties"][k], externalData);
          }else{
