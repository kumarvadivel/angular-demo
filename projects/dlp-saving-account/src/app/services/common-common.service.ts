...
import { LogicFunctions } from '@sba-app/shared/helpers/JsonLogic';
import { ArrayMethod } from '@sba-app/shared/helpers/ArrayMethods';
import { LocalStorage } from '@sba-app/shared/helpers/LocalStorage';
import { ApiService } from './api.service';
import { InitializeJourneyService } from '@sba-app/services/initialize-journey.service';
import { assign, uniqBy } from '@sba-app/shared/utils/utils';
...
export class CommonCommonService {
  ...
  getDocumentList(field?){
    ...
    return new Promise((resolve, _reject) => { 
      this.apiService.getUploadedDoc(param).then((resp:any)=>{
        if(resp){
          resolve(resp)
          resp.supportingDocuments?.forEach((data) => {
            this.commonVariableService.reqSupportingDocList.forEach((item) => {
                if (data.DocumentCategoryUuid == item.documentTypeUuid) {
                  if (item.loanPurposeDocumentCategoryCode == 'IND_PAP' && data.documentUploadedSuccessfully && data.size > 0 ) {
                        this.commonVariableService.globalLoaderState = false;
                        this.commonVariableService.uploadedDoc = data     
                        this.commonVariableService.uploaddone = true
                        field.value = data     
                  }
                }
            });
  ...
  formDataEditDataPopulatorSubMethod(section, data, metaConfig){
    let mapping_Data = {}
      ...
      for (const prop in metaConfig.prepopulationRemaps) {
        if (Array.isArray(metaConfig.prepopulationRemaps[prop])) {
          mapping_Data[prop] = this.ArrayMethods.getPathVal(data, this.applyJsonLogic(metaConfig.prepopulationRemaps[prop][0], data))
           for(const val in  metaConfig.prepopulationRemaps[prop][1]){
            if(Array.isArray(metaConfig.prepopulationRemaps[prop][1][val])){
              metaConfig.prepopulationRemaps[prop][1][val]=this.applyJsonLogic(metaConfig.prepopulationRemaps[prop][1][val][0],data)
    ...
    config.map(section => {
      if (section.componentType == 'FORM') {
        if (metaConfig.prepopulationRemaps) {
          this.formDataEditDataPopulatorSubMethod(section, data, metaConfig)
        }
      }
    })
